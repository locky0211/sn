...
function add() {
	mini.open({
		url : base + 'ews/jsp/checkFormula.jsp',
		title : '校验公式维护',
		width : 660,
		height : 420,
		allowResize : false,
...
function edit(e) {
	...
	mini.open({
		url : base + 'formula/getFormulaByid.nut?id=' + e.id + '&page=/ews/jsp/checkFormula.jsp',
		title : '客户名称维护',
		width : 660,
		height : 420,
		allowResize : false,
...
function chageState() {
	...
	$.ajax({
		type : 'POST',
		dataType : 'json',
		url : base + 'formula/updateFormulaState.nut?id=' + e.id,
		success : function(text) {
			if (text.success) {
		...
		mini.confirm("确定删除记录？", "确定？", function(action) {
			if (action == 'ok') {
				$.ajax({
					type : 'POST',
					dataType : 'json',
					url : base + 'formula/deleteFormula.nut?id=' + e.id,
					success : function(text) {
						if (text) {
