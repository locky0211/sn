...
function edit(e) {
	...
	mini.open({
		url : base + 'sjscore/formula/getFormulaByCode.nut?formulaCode=' + e.formulaCode + '&page=/ews/jsp/scoreFormula_sj.jsp?falg=2',
		title : '打分公式维护',
		width : $(top.window).width()-500,
		height : $(top.window).height()-220,
		allowResize : false,
		...
		mini.confirm("确定删除记录？", "确定？", function(action) {
			if (action == 'ok') {
				$.ajax({
					type : 'POST',
					dataType : 'json',
					url : base + 'sjscore/formula/deleteFormula.nut?formulaCode=' + e.formulaCode,
