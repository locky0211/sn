		...
		mini.confirm("确定取消校验？", "确定？", function(action) {
			if (action == 'ok') {
				...
				$.ajax({
					type : 'POST',
					dataType : 'json',
					url : base + 'check/allInfoSls/zdy/cancelCheck.nut?id=' + e.id,
					success : function(text) {
						if (text) {
		...
		mini.confirm("确定删除记录？", "确定？", function(action) {
			if (action == 'ok') {
				...
				$.ajax({
					type : 'POST',
					dataType : 'json',
					url : base + 'check/allInfoSls/zdy/delete.nut?id=' + e.id,
					success : function(text) {
						if (text) {
...
function chageState() {
	...
	$.ajax({
		type : 'POST',
		dataType : 'json',
		url : base + 'check/allInfoSls/zdy/updateState.nut?id=' + e.id,
		success : function(text) {
			if (text.success) {
				if(e.status=="1")
				{
...
function rowdblclick() {
	var e = checkGrid.getSelected();
		mini.open({
			url : base + 'ews/jsp/checkAllInfoResult_sls_zdy.jsp?brNo=' + e.brNO + '&reportDate=' + e.sjRQ+'&formulaType=' +e.formulaType+'&version=' +e.version+'&tableRange=' +e.tableRange,
			title : "校验结果明细",
			width : 1200,
			height : 600,
			allowResize : true
...
function addCheck(){
	if (mini.get("brNo").getValue() == '' || mini.get("brNo").getValue() == null) {
		...
		$.ajax({
			type : 'POST',
			dataType : 'json',
			url : base + 'check/allInfoSls/zdy/addCheckAllInfo.nut?brNo=' + mini.get("brNo").getValue() + '&reportDate=' + mini.get("reportDate").getText() + '&version=' + mini.get("version").getValue()+'&formulaType=' + mini.get("formulaType").getValue()+'&tableRange=' + mini.get("tableRange").getValue(),
			success : function(text) {
				if (text.success) {
...
function addAllCheck(){
	if (mini.get("brNo").getValue() == '' || mini.get("brNo").getValue() == null) {
		...
		$.ajax({
			type : 'POST',
			dataType : 'json',
			url : base + 'check/allInfoSls/zdy/addAllCheck.nut?brNo=' + mini.get("brNo").getValue() + '&reportDate=' + mini.get("reportDate").getText() + '&version=' + mini.get("version").getValue()+'&formulaType=' + 
			mini.get("formulaType").getValue()+'&tableRange=' + mini.get("tableRange").getValue()+'&isNotAllCheck='+true,
			success : function(text) {
				if (text.success) {
					mini.alert(JSON.stringify(text.data), '提醒', function() {
						checkTempGrid.reload();
...
function reCheckAllInfo() {
	$.ajax({
		type : 'POST',
		dataType : 'json',
		url : base + 'check/allInfoSls/zdy/reCheckAllInfo.nut?reportDate=' + mini.get("reportDate").getText() + '&version=' + mini.get("version").getValue()+'&formulaType=' + mini.get("formulaType").getValue()+'&tableRange=' + mini.get("tableRange").getValue(),
		success : function(text) {
			if (text.success) {
				checkTempGrid.reload();
				checkGrid.reload();
...
function reCheckAllInfoForAll() {
	$.ajax({
		type : 'POST',
		dataType : 'json',
		url : base + 'check/allInfoSls/zdy/reCheckAllInfo.nut?reportDate=' + mini.get("reportDate").getText() + '&version=' + mini.get("version").getValue()+'&formulaType=' + mini.get("formulaType").getValue()+'&tableRange=' + mini.get("tableRange").getValue()+'&isNotCheckAll='+true,
		success : function(text) {
			if (text.success) {
				checkTempGrid.reload();
				checkGrid.reload();
...
function search() {
	...
	$.ajax({
		type : 'POST',
		dataType : 'json',
		url : base + 'check/allInfoSls/zdy/closeRefresh.nut',
		success : function(text) {
			if (text.success) {
				clearInterval(t);
				t=null;
