...
//	dt.setDate(1);
//	cdt = new Date(dt.getTime() - 1000 * 60 * 60 * 24);
//	// mini.get('#checkDate').setValue(cdt.getFullYear() + '-' +
//	// (Number(cdt.getMonth()) + 1) + "-" + cdt.getDate());
//	mini.get('#reportDate').setValue(cdt.getFullYear() + '-' + (Number(cdt.getMonth()) + 1) + "-" + cdt.getDate());
	checkGrid.load({
		brNo : mini.get('brNo').getValue(),
		//reportDate : mini.get('reportDate').getFormValue(),
		...
		checkType : mini.get('checkType').getFormValue()
	});
	}
	$.ajax({
		type : 'POST',
...
function excute() {
	if (mini.get("brNo").getValue() == '' || mini.get("brNo").getValue() == null) {
		...
		$.ajax({
			type : 'POST',
			dataType : 'json',
			url : base + 'check/notnull/startCheck.nut?brNo=' + mini.get("brNo").getValue() + '&reportDate=' + mini.get("reportDate").getText() +'&version=' + mini.get("version").getValue() + '&checkType=' + mini.get("checkType").getValue(),
			success : function(text) {
				if (text.success) {
...
function excute_sd() {
	if (mini.get("brNo").getValue() == '' || mini.get("brNo").getValue() == null) {
		...
		$.ajax({
			type : 'POST',
			dataType : 'json',
			url : base + 'check/notnullsd/startCheck.nut?brNo=' + mini.get("brNo").getValue() + '&startDate=' + mini.get("startDate").getText() + '&endDate=' + mini.get("endDate").getText()+'&version=' + mini.get("version").getValue() + '&checkType=' + mini.get("checkType").getValue(),
			success : function(text) {
				if (text.success) {
					mini.alert(JSON.stringify(text.data), '提醒', function() {
						checkGrid.load({
...
function search() {
	...
	checkGrid.load({
		...
		checkType : mini.get('#checkType').getValue()
	});
}
...
function a(){
	$.ajax({
		type : 'POST',
		dataType : 'json',
		url : base + 'check/notnull/refreshSession.nut',
		success : function(text) {
			if (text.success) {
				var data1=text.data;
				mini.mask({
...
function a_sd(){
	$.ajax({
		type : 'POST',
		dataType : 'json',
		url : base + 'check/notnullsd/refreshSession.nut',
		success : function(text) {
			if (text.success) {
				var data1=text.data;
				mini.mask({
...
function exportExcel() {
   ...
   mini.mask({
	...
	$.ajax({
		...
		data :"brnoName=" + brnoName+"&brno="+brno+"&reportDate="+reportDate+"&tableName=CHECK_NOTNULL",
		dataType : 'json',
...
//	var reportDate=mini.get('#reportDate').getText();
   ...
   mini.mask({
	...
	$.ajax({
		...
		data :"brnoName=" + brnoName+"&brno="+brno+"&startDate="+startDate+"&tableName=CHECK_NOTNULL",
		dataType : 'json',
		success : function(data) {
			toDownLoadFileByFilePath(data);
		},
...
function searchCheckLog(){
	var brNo = mini.get("brNo").getValue();
	mini.open({
		url : base + 'ews/jsp/checklog.jsp?brNo='+brNo+'&checkType='+1,
		title : "校验日志",
		width : 1000,
		height : 200,
		allowResize : true
