...
function rowdblclick(e) {
	var record = e.record;
	mini.open({
		url : base + 'formula/getFormulaById.nut?id=' + record.formulaId + '&page=/ews/jsp/checkResult.jsp?brNo=' + mini.get("brNo").getValue() + '&reportDate=' + mini.get("reportDate").getText()
...
function excute() {
	if (mini.get("brNo").getValue() == '' || mini.get("brNo").getValue() == null) {
		...
		$.ajax({
			type : 'POST',
			dataType : 'json',
			url : base + 'check/complex/startCheck.nut?brNo=' + mini.get("brNo").getValue() + '&reportDate=' + mini.get("reportDate").getText()+ '&version=' + mini.get("version").getValue(),
			success : function(text) {
				if (text.success) {
...
function excute_sd() {
	if (mini.get("brNo").getValue() == '' || mini.get("brNo").getValue() == null) {
		...
		$.ajax({
			type : 'POST',
			dataType : 'json',
			url : base + 'check/complexsd/startCheck.nut?brNo=' + mini.get("brNo").getValue() + '&startDate=' + mini.get("startDate").getText() + '&endDate=' + mini.get("endDate").getText()+ '&version=' + mini.get("version").getValue(),
			success : function(text) {
				if (text.success) {
					mini.alert(JSON.stringify(text.data), '提醒', function() {
						checkGrid.load({
...
function a(){
	$.ajax({
		type : 'POST',
		dataType : 'json',
		url : base + 'check/complex/refreshSession.nut',
		success : function(text) {
			if (text.success) {
				var data1=text.data;
				mini.mask({
...
function a_sd(){
	$.ajax({
		type : 'POST',
		dataType : 'json',
		url : base + 'check/complexsd/refreshSession.nut',
		success : function(text) {
			if (text.success) {
				var data1=text.data;
				mini.mask({
...
function exportExcel() {
	...
	$.ajax({
		...
		data :"brnoName=" + brnoName+"&brno="+brno+"&reportDate="+reportDate+"重点"+"&tableName=CHECK_COMPLEX",
		dataType : 'json',
...
function exportExcel_sd() {
	...
	$.ajax({
		...
		data :"brnoName=" + brnoName+"&brno="+brno+"&startDate="+startDate+"&tableName=CHECK_COMPLEX",
		dataType : 'json',
		success : function(data) {
			toDownLoadFileByFilePath(data);
		},
...
function searchCheckLog(){
	var brNo = mini.get("brNo").getValue();
	mini.open({
		url : base + 'ews/jsp/checklog.jsp?brNo='+brNo+'&checkType='+6,
		title : "校验日志",
		width : 1000,
		height : 200,
		allowResize : true
