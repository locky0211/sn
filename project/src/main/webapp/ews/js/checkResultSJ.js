...
function rowdblclick(e) {
	var record = e.record;
	mini.open({
		url : base + 'formula/getFormulaById.nut?id=' + record.formulaId + '&page=/ews/jsp/checkResult.jsp?brNo=' +record.bNo + '&reportDate=' + record.sjrq
...
function excute() {
	...
	if (mini.get("brNo").getValue() == '' || mini.get("brNo").getValue() == null) {
		...
		$.ajax({
			type : 'POST',
			dataType : 'json',
			url : base + 'check/resultsj/startCheck.nut?brNo=' + mini.get("brNo").getValue() + '&reportDate=' + mini.get("reportDate").getText(),
			//url : base + 'check/resultsj/startSJCheck.nut',
...
function excute_sd() {
	if (mini.get("brNo").getValue() == '' || mini.get("brNo").getValue() == null) {
		...
		$.ajax({
			type : 'POST',
			dataType : 'json',
			url : base + 'check/complexsd/startCheck.nut?brNo=' + mini.get("brNo").getValue() + '&startDate=' + mini.get("startDate").getText() + '&endDate=' + mini.get("endDate").getText(),
			success : function(text) {
				if (text.success) {
					mini.alert(JSON.stringify(text.data), '提醒', function() {
						checkGrid.load({
...
function exportExcel() {
	...
	var checkName = mini.get('#repCode').getValue();
	var brName = mini.get('#brNo').getText();
	mini.mask({
		el : document.body,
		cls : 'mini-mask-loading',
		html : '处理中...'
	...
	$.ajax({
		type : "POST",
		url : base + "check/resultsj/exportExcel.nut",
		data :"brName="+brName+"&checkName=" + checkName+"&brno="+brno+"&reportDate="+reportDate+"&tableName=CHECK_RESULT_SJ",
		dataType : 'json',
