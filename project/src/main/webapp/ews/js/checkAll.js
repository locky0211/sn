...
function excute() {
	if (mini.get("brNo").getValue() == '' || mini.get("brNo").getValue() == null) {
		...
		$.ajax({
			type : 'POST',
			dataType : 'json',
			url : base + 'check/all/startCheck.nut?brNo=' + mini.get("brNo").getValue() + '&reportDate=' + mini.get("reportDate").getText() + '&version=' + mini.get("version").getValue()+ '&repCode=' + mini.get("repCode").getValue()+'&formulaType=' + mini.get("formulaType").getValue(),
			success : function(text) {
				if (text.success) {
...
function excute_sd() {
	if (mini.get("brNo").getValue() == '' || mini.get("brNo").getValue() == null) {
		...
		$.ajax({
			type : 'POST',
			dataType : 'json',
			url : base + 'check/allsd/startCheck.nut?brNo=' + mini.get("brNo").getValue() + '&startDate=' + mini.get("startDate").getText() + '&endDate=' + mini.get("endDate").getText() + '&version=' + mini.get("version").getValue()+ '&repCode=' + mini.get("repCode").getValue()+'&formulaType=' + mini.get("formulaType").getValue(),
			success : function(text) {
				if (text.success) {
					mini.alert(JSON.stringify(text.data), '提醒', function() {
						checkGrid.load({
...
function a_sd(){
	$.ajax({
		type : 'POST',
		dataType : 'json',
		url : base + 'check/allsd/refreshSession.nut',
		success : function(text) {
			if (text.success) {
				var data1=text.data;
				mini.mask({
...
function searchCheckLog(){
	var brNo = mini.get("brNo").getValue();
	mini.open({
		url : base + 'ews/jsp/checklog.jsp?brNo='+brNo+'&checkType='+7,
		title : "校验日志",
		width : 1000,
		height : 200,
		allowResize : true
