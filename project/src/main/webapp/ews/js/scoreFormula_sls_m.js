...
function add() {
	mini.open({
		url : base + 'ews/jsp/scoreFormulaSls.jsp?flag=1',
		title : '新增打分公式',
...
function edit(e) {
	...
	mini.open({
		url : base + 'score/formula/sls/getFormulaByCode.nut?formulaCode=' + e.formulaCode + '&page=/ews/jsp/scoreFormulaSls.jsp?falg=2',
		title : '打分公式维护',
		width : $(top.window).width()-500,
		height : $(top.window).height()-100,
		allowResize : false,
		...
		mini.confirm("确定删除记录？", "确定？", function(action) {
			if (action == 'ok') {
				$.ajax({
					type : 'POST',
					dataType : 'json',
					url : base + 'score/formula/sls/deleteFormula.nut?formulaCode=' + e.formulaCode,
