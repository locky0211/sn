...
function zbbhIsOnly(e){
	...
	if(zbbh == ''){
		if(mini.get("zbbh").getValue() != ''){
			$.ajax({
				url : base + "scoreZfphSql/checkIsOnly.nut?zbbh=" + mini.get("zbbh").getValue(),
				dataType : 'json',
				async : false,
	...
	if (form.isValid()) {
		...
		if (zbbh == '') {
			$.ajax({
				type : 'POST',
				dataType : 'json',
				url : base + 'scoreZfphSql/addScoreZfphSql.nut',
				data : json,
			...
			$.ajax({
				type : 'POST',
				dataType : 'json',
				url : base + 'scoreZfphSql/updateScoreZfphSql.nut',
				data : json,
				success : function(text) {
					if (text) {
						mini.alert('操作成功!!', '提醒', function() {
