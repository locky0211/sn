...
function excute() {
	if (mini.get("brNo").getValue() == '' || mini.get("brNo").getValue() == null) {
		...
		$.ajax({
			type : 'POST',
			dataType : 'json',
			url : base + 'check/length/startCheck.nut?brNo=' + mini.get("brNo").getValue() + '&reportDate=' + mini.get("reportDate").getText()+ '&version=' + mini.get("version").getValue(),
			success : function(text) {
				if (text.success) {
...
function excute_sd() {
	if (mini.get("brNo").getValue() == '' || mini.get("brNo").getValue() == null) {
		...
		$.ajax({
			type : 'POST',
			dataType : 'json',
			url : base + 'check/lengthsd/startCheck.nut?brNo=' + mini.get("brNo").getValue() + '&startDate=' + mini.get("startDate").getText() + '&endDate=' + mini.get("endDate").getText()+ '&version=' + mini.get("version").getValue(),
			success : function(text) {
				if (text.success) {
					mini.alert(JSON.stringify(text.data), '提醒', function() {
						checkGrid.load({
...
function a(){
	$.ajax({
		type : 'POST',
		dataType : 'json',
		url : base + 'check/length/refreshSession.nut',
		success : function(text) {
			if (text.success) {
				var data1=text.data;
				mini.mask({
...
function a_sd(){
	$.ajax({
		type : 'POST',
		dataType : 'json',
		url : base + 'check/lengthsd/refreshSession.nut',
		success : function(text) {
			if (text.success) {
				var data1=text.data;
				mini.mask({
...
function exportExcel() {
	...
	$.ajax({
		...
		data :"brnoName=" + brnoName+"&brno="+brno+"&reportDate="+reportDate+"&tableName=CHECK_LENGTH",
		dataType : 'json',
...
//	var reportDate=mini.get('#reportDate').getText();
	...
	$.ajax({
		...
		data :"brnoName=" + brnoName+"&brno="+brno+"&startDate="+startDate+"长度"+"&tableName=CHECK_LENGTH",
		dataType : 'json',
		success : function(data) {
			toDownLoadFileByFilePath(data);
		},
...
function searchCheckLog(){
	var brNo = mini.get("brNo").getValue();
	mini.open({
		url : base + 'ews/jsp/checklog.jsp?brNo='+brNo+'&checkType='+2,
		title : "校验日志",
		width : 1000,
		height : 200,
		allowResize : true
