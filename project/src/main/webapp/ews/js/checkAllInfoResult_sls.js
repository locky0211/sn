...
function rowdblclick(e) {
	var record = e.record;
		mini.open({
			url : base + 'formula/getFormulaById.nut?id=' + record.formulaId + '&page=/ews/jsp/checkResultSls.jsp?brNo=' + mini.get("brNo").getValue() + '&reportDate=' + mini.get("reportDate").getText() + '&fieldCode=' + record.fieldCode+'&nbjgh=' + record.NBJGH,
			title : "检验结果明细",
			width : 1200,
			height : 600,
			allowResize : true
...
function exportExcel() {
	...
	$.ajax({
		...
		data :"brnoName=" + brnoName+"&brno="+brno+"&reportDate="+reportDate+"&tableName=CHECK_ALL_SLS"+"&repCode="+repCode+"&fBrno="+fbrNo,
		dataType : 'json',
...
function reCheck() {
	...
	$.ajax({
		type : 'POST',
		dataType : 'json',
		url : base + 'check/allInfoSls/singleFormulaCheckJN.nut?formulaId=' + e.formulaId+'&brNo='+mini.get("brNo").getValue()+'&reportDate='+ mini.get('reportDate').getText()+'&id='+ e.id+'&nbjgh=' + e.NBJGH,
		success : function(text) {
			if (text.success) {
				mini.alert(JSON.stringify(text.data), '提醒', function() {
					checkGrid.reload();
...
function exportExcelForOrgan() {
	...
	$.ajax({
		type : "POST",
		url : base + "getorganinfo/exportExcelForOrgan.nut",
...
function exportExcelForOrganOrDownSls() {
	var form=new mini.Form("#checkForm");
	form.validate();
	var brno=mini.get('#brNo').getValue();
	var reportDate=mini.get('#reportDate').getText();
	...
	$.ajax({
		type : "POST",
		url : base + "getorganinfo/exportExcelForOrganOrDownSls.nut",
		data :"brno=" + brno+"&reportDate="+reportDate+"&repCode="+repCode+"&formulaType="+formulaType+"&fBrno="+fbrNo,
		dataType : 'json',
		success : function(text) {
			if (text.success) {
