...
function add() {
	mini.open({
		url : base + 'ews/jsp/score_zfph_sql_sls.jsp',
		title : '校验公式维护',
		width : 660,
		height : 400,
		allowResize : false,
		...
		mini.confirm("确定删除记录？", "确定？", function(action) {
			if (action == 'ok') {
				$.ajax({
					type : 'POST',
					dataType : 'json',
					url : base + 'scoreZfphSqlSls/deleteScoreZfphSqlSls.nut?zbbh=' + e.zbbh,
...
function edit(e) {
	var e = scoreZfpgGrid.getSelected();
	mini.open({
		url : base + 'scoreZfphSqlSls/getScoreById.nut?zbbh=' + e.zbbh + '&page=/ews/jsp/score_zfph_sql_sls.jsp',
		title : '校验公式维护',
		width : 660,
		height : 400,
		allowResize : false,
...
function chageState(){
	...
	$.ajax({
		type : 'POST',
		dataType : 'json',
		url : base + 'scoreZfphSqlSls/updateSqlState.nut?zbbh=' + e.zbbh,
		success : function(text) {
			if (text.success) {
