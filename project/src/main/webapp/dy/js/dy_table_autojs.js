...
function toCreateAutoJs() {
	...
	if (rows.length > 0) {
		...
		$.ajax({
					url : base + 'dy/autojs/toGenerationAutoJs.nut?tabCodes=' + dataStr.substring(0, dataStr.length - 1) + '&allVersion='
...
function doImportAutoJs() {
	if (mini.get('excelFile').getValue() == '' || mini.get('excelFilePath').getValue() == '') {
		...
		$.ajax({
					url : base + 'dy/autojs/toUploadAutoJsFile.nut?excelFile=' + mini.get('excelFile').getValue() + '&excelFilePath='
