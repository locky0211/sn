...
function add() {
	...
	for(var i=0;i<size;i++){
		...
		if(i<size-1){
			jsonstr += "{\"id\""+ ":" + "\"" + ids + "\",\"checkRisk\""+ ":" + "\"" + cr + "\",\"reportRate\"" + ":" + "\"" + rr + "\",\"element\""+ ":" + "\"" + e + "\",\"lastElement\""+ ":" + "\"" + le + "\",\"leftValue\""+ ":" + "\"" + lV + "\",\"rightValue\""+ ":" + "\"" + rV + "\",\"thisVersion\""+ ":" + "\"" + sd + "\",\"lastVersion\""+ ":" + "\"" + ed + "\",\"isDel\""+ ":" + "\"" + iD + "\"},";
		}else{
			jsonstr += "{\"id\""+ ":" + "\"" + ids + "\",\"checkRisk\""+ ":" + "\"" + cr + "\",\"reportRate\"" + ":" + "\"" + rr + "\",\"element\"" + ":" + "\"" + e + "\",\"lastElement\""+ ":" + "\"" + le + "\",\"leftValue\""+ ":" + "\"" + lV + "\",\"rightValue\""+ ":" + "\"" + rV + "\",\"thisVersion\""+ ":" + "\"" + sd + "\",\"lastVersion\""+ ":" + "\"" + ed + "\",\"isDel\""+ ":" + "\"" + iD + "\"}";
		}
	}
	...
	if (form.isValid()) {
			...
			$.ajax({
						url : base + 'dy/check/wave/addOrUpdateYiDongCheckFormulaCS.nut',
						type : 'post',
...
function checkIsOnly(e) {
	...
	if (regu.test(e.value)) {
		...
		// 唯一性校验
		$.ajax({
					url : base + "dy/check/checkIsOnly.nut?checkFormula=" + e.value + "&id=" + mini.get("id").getValue(),
					async : false,
					dataType : 'json',
...
//	inner_html.push('</td>');
	...
	entry+="<td><input id='id"+ size +"' name='id"+ size +"' class='mini-hidden'  value=''/>"
			+"<input id='checkRisk"+ size +"' name='checkRisk"+ size +"' class='mini-combobox' style='width: 100px;'  popupWidth='100' textField='zdName' valueField='zdValue' url=\"../../../sys/ggzd/getGgzdList.nut?groupId=WAVE_CHECK_RISK\" required='true' multiSelect='false' allowInput='false' showClose='true' oncloseclick='onCloseClick'/></td>";
	entry+="<td><input id='reportRate"+ size +"' name='reportRate"+ size +"' class='mini-combobox' style='width: 100px;'  popupWidth='100' textField='zdName' valueField='zdValue' url=\"../../../sys/ggzd/getGgzdList.nut?groupId=DY_YIDONG_FORMULA_TYPE\" required='true' multiSelect='true' allowInput='false' showClose='true' oncloseclick='onCloseClick'/></td>";
	...
	entry+="<td><input id='thisVersion"+ size +"' name='thisVersion"+ size +"'  style='width: 100px;' showClose='true' oncloseclick='onCloseClick' class='mini-combobox' url=\"../../../dy/table/getTableVersionNo.nut?tabCode="+tabCode+"\" textField='versionNo' multiSelect='false' valueField='versionNo'  resultAsTree='false' allowInput='true' expandOnNodeClick='true'/></td>";
	entry+="<td><input id='lastVersion"+ size +"' name='lastVersion"+ size +"'  style='width: 100px;' showClose='true' oncloseclick='onCloseClick' class='mini-combobox' url=\"../../../dy/table/getTableVersionNo.nut?tabCode="+tabCode+"\" textField='versionNo' multiSelect='false' valueField='versionNo'  resultAsTree='false' allowInput='true' expandOnNodeClick='true'/></td>";
	entry+="<td><a class='mini-button' onclick=\"delCheckFormula(\'"+ size +"\')\" style='width: 40px;'>删除</a>";
	entry+="<input class='mini-hidden' id='isDel"+ size +"' name='isDel"+ size +"' value='0' style='width: 10px;' /></td>"
	entry+="</tr>";
