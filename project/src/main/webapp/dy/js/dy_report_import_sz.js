...
function downLoadExcel(reportId) {
	...
	$.ajax({
			...
			url : base + 'dy/report/importSZ/doExportExcelSZ.nut',
			type : 'post',
			dataType : 'json',
			data : {
				reportId : reportId
...
function toReportView(row_uid) {
	...
	mini.open({
				url : base + "dy/report/importSZ/toDyTableReportSZ.nut?reportId=" + row.reportId + "&tableId=" + row.tableId,
...
function doReportImport() {
	...
	if (form.isValid()) {
		if (mini.get('excelFile').getValue() == '' || mini.get('excelFilePath').getValue() == '') {
			...
			$.ajax({
						url : base + 'dy/report/importSZ/doReportImportSZ.nut',
						type : 'post',
						dataType : 'json',
						data : json,
						cache : false,
