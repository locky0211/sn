...
function toReportView(row_uid) {
	...
	if(reportDate!=null&&reportDate!=""){
		mini.open({
					url : base + "dy/table/report/toRdTableFormulaReportCS.nut?tabCode=" + row.tabCode + "&reportDate=" + reportDate +"&cUser=" + cUser,
					title : row.tabName,
					iconCls : "icon-excel",
...
function autoAddCheckFormula(){
	...
	if (rows.length == 1) {
	    ...
	    mini.open({
	    	type : 'POST',
	    	url : base + 'dy/jsp/autoAddFormulaYidong_cs.jsp?reportDate=' + reportDate + '&tabCode=' + tabcode + '&tabType=' + tabType + '&cUser=' + cUser,
	    	title : '批量公式',
	    	iconCls : 'icon-edit',
	    	width : 400,
	    	height : 200,
...
function autoAddCheckFormulaVersion(){
	...
	if (rows.length == 1) {
	    ...
	    mini.open({
	    	type : 'POST',
	    	url : base + 'dy/jsp/autoAddFormulaYidong_version_cs.jsp?reportDate='+reportDate+'&tabCode='+tabcode+'&cUser='+cUser,
	    	title : '批量公式',
	    	iconCls : 'icon-edit',
	    	width : 420,
	    	height : 400,
