...
function showTableImportView(row_uid) {
	...
	mini.open({
		url : base + 'dy/jsp/dy_check_table_import.jsp?brNo=' + row.field1 + '&tabType=' + mini.get("tabType").getValue() + '&reportDate='
...
function showCheckView(brNo, checkRisk) {
	parent.showTab(brNo, '校验结果信息', base + 'dy/jsp/dy_check_result_list.jsp?brNo=' + brNo + '&reportDate=' + mini.get("rDate").getFormValue()
