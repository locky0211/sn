...
function exportExcel() {
	...
	$.ajax({
		type : "POST",
		url : base + "dy/check/exportExcel.nut",
		data : "id=" + dataStr,
		dataType : 'json',
...
function importExcel(){
	mini.open({
		url : base + 'dy/jsp/dy_check_formula_import.jsp',
		title : "导入校验公式", 
		iconCls : "icon-add",
		width : 352,
		height : 258,
...
function rollback(){
	mini.open({
		url : base + 'dy/jsp/dy_check_formula_rollback.jsp',
		title : "回滚校验公式", 
		iconCls : "icon-add",
		width : 415,
		height : 500,
...
function toUpdateCheckFormulaStatus(row_uid) {
	...
	$.ajax({
				type : 'POST',
				url : base + 'dy/check/updateCheckFormulaStatus.nut',
				data : {
					'id' : row.id,
...
function addCheckFormula(e) {
	mini.open({
				url : base + "dy/jsp/dy_check_formula.jsp?cUser=" + $('#cUserInput').val(),
				title : "新增校验公式",
				iconCls : "icon-add",
				width : 580,
				height : 450,
...
function edit() {
	...
	if (row) {
		mini.open({
					url : base + 'dy/check/toEditCheckFormula.nut?id=' + row.id + '&page=/dy/jsp/dy_check_formula.jsp',
					title : '校验公式修改',
					iconCls : 'icon-edit',
					width : 580,
					height : 450,
...
function updateRow(row) {
	$.ajax({
				type : 'POST',
				url : base + 'dy/check/fetchCheckFormula.nut',
				data : {
					'id' : row.id
				},
				dataType : 'json',
	...
	if (row) {
		mini.confirm('确定删除？删除后无法恢复!', '确定？', function(action) {
					if (action == 'ok') {
						$.ajax({
									type : 'POST',
									url : base + 'dy/check/deleteCheckFormula.nut',
									data : {
										'id' : row.id
									},
									dataType : 'json',
