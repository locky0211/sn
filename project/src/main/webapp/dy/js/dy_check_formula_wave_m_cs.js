...
function toUpdateCheckFormulaStatus(row_uid) {
	...
	$.ajax({
				type : 'POST',
				url : base + 'dy/check/wave/updateCheckFormulaStatus.nut',
				data : {
					'id' : row.id,
...
function addCheckFormula(e) {
	mini.open({
				url : base + "dy/jsp/dy_check_formula_wave_cs.jsp",
				title : "新增校验公式",
				iconCls : "icon-add",
				width : 530,
				height : 420,
...
function edit() {
	...
	if (row) {
		mini.open({
					url : base + 'dy/check/wave/toEditCheckFormulaCS.nut?id=' + row.id + '&page=/dy/jsp/dy_check_formula_wave_cs.jsp',
					title : '校验公式修改',
					iconCls : 'icon-edit',
					width : 530,
					height : 420,
...
function updateRow(row) {
	$.ajax({
				type : 'POST',
				url : base + 'dy/check/wave/fetchCheckFormulaWave.nut',
				data : {
					'id' : row.id
				},
				dataType : 'json',
	...
	if (row) {
		mini.confirm('确定删除？删除后无法恢复!', '确定？', function(action) {
					if (action == 'ok') {
						$.ajax({
									type : 'POST',
									url : base + 'dy/check/wave/deleteCheckFormulaCS.nut',
									data : {
										'id' : row.id
									},
									dataType : 'json',
...
function doAutoInsert(){
	mini.open({
		url : base + "dy/jsp/dy_table_list_cs.jsp",
		title : '查看执行步骤',
		iconCls : 'icon-edit',
		width : 800,
		height : 500,
