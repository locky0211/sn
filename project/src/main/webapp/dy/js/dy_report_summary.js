...
function downLoadExcel(reportId) {
	...
	$.ajax({
				url : base + 'dy/report/Summary/doExportExcel.nut',
				type : 'post',
				dataType : 'json',
				data : {
					reportId : reportId
...
function downLoadExcels() {
	...
	var rows = grid.getSelecteds();
            if (rows.length > 0) {
                ...
                toDownLoadFile(base + "dy/report/import/downloadAllExcelFiles.nut?ids="+ids);
            } else {
                alert("请选中至少一条记录!");
            }
}
...
function toReportView(row_uid) {
	...
	mini.open({
				url : base + "dy/report/Summary/toDyTableReport.nut?reportId=" + row.reportId + "&tableId=" + row.tableId,
...
function doReportSummary() {
	...
	if (form.isValid()) {
			...
			$.ajax({
				url : base + 'dy/report/Summary/DyReportSummary.nut',
				type : 'post',
				dataType : 'json',
				data : json,
				cache : false,
...
function doPartTask() {
	...
	mini.open({
		url : base + "dy/jsp/dy_report_summary_taskList.jsp?&brNo="+brNo+"&reportDate="+reportDate+"&tabType="+tabType,
		title : '查看执行步骤',
		iconCls : 'icon-edit',
		width : 800,
		height : 500,
