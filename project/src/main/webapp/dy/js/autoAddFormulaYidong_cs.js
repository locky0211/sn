...
function add(){
	...
	if(form.isValid()&&checkIsNumber()){
		if(typeCode()){
			...
			$.ajax({
						url : base + 'dy/check/wave/autoAddFormulaYidongCS.nut?tabCode='+tabCode+'&checkRisk='+checkRisk+'&reportRate='+reportRate+'&leftValue='+leftValue+'&rightValue='+rightValue+'&thisVersion='+thisVersion+'&cUser='+cUser,
						type : 'post',
						dataType : 'json',
						cache : false,
