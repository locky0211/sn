...
function toUpdateTableInfoStatus(row_uid) {
	...
	$.ajax({
				type : 'POST',
				url : base + 'dy/table/updateTableInfoStatus.nut',
				data : {
					'tabCode' : row.tabCode,
					'status' : row.status
				},
...
function addTableInfo(e) {
	mini.open({
				url : base + "dy/jsp/dy_table_info_wj.jsp",
				title : "新增报表模板信息",
				iconCls : "icon-add",
				width : 550,
				height : 600,
...
function addTableInfoVersionNo(e) {
	...
	if (row) {
		mini.open({
					url : base + 'dy/table/toAddTableInfoVersionNo.nut?tabCode=' + row.tabCode + '&page=/dy/jsp/dy_table_info.jsp',
					title : '新增报表模板信息',
					iconCls : 'icon-add',
					width : 550,
					height : 600,
...
function updateRow(row) {
	$.ajax({
				type : 'POST',
				url : base + 'dy/table/fetchTableInfo.nut',
				data : {
					'tabCode' : row.tabCode
				},
				dataType : 'json',
...
function update() {
	...
	if (row) {
		mini.open({
					url : base + 'dy/jsp/dy_table_info_update_wj.jsp?tabCode=' + row.tabCode,
					title : '报表模板信息修改',
					iconCls : 'icon-edit',
					width : 850,
					height : 350,
	...
	if (row) {
		mini.confirm('删除后可能影响系统正常运行!', '确定？', function(action) {
					if (action == 'ok') {
						$.ajax({
									type : 'POST',
									url : base + 'dy/table/deleteTableBasicInfo.nut',
									data : {
										'tabCode' : row.tabCode
									},
									dataType : 'json',
