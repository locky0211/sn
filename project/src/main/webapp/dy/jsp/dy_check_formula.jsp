...
<script type="text/javascript" src="../js/dy_check_formula.js"></script>
</head>
	...
	<form id="checkformulaForm" method="post">
		...
		<div>
			<table cellpadding="4" cellspacing="4" style="padding-left: 12px;">
				...
				<tr>
					<td>风险等级：</td>
					<td><input name="checkRisk" class="mini-combobox" required="true" style="width: 150px;" textField="zdName" valueField="zdValue"
						url="${base }/sys/ggzd/getGgzdList.nut?groupId=DY_CHECK_RISK" value="${obj.checkRisk }" allowInput="false" /></td>
					<td>容忍波动值：</td>
					<td><input id="deviationValue" name="deviationValue" class="mini-textbox" value="${obj.deviationValue }"
				...
				<tr>
					...
					<td><input id="type" name="type" class="mini-combobox" required="true" style="width: 150px;" textField="typeName" valueField="typeValue"
						data="[{typeValue:'1',typeName:'普通'}]" value="${obj.type }" allowInput="false" /></td>
					<td>校验区域:</td>
					<td><input name="checkArea" class="mini-combobox" required="true" style="width: 80px;" textField="zdName" valueField="zdValue"
						url="${base }/sys/ggzd/getGgzdList.nut?groupId=CHECK_AREA" value="${obj.checkArea }" allowInput="false"/>
		...
		<div style="text-align: center; padding: 10px;">
			<a class="mini-button" onclick="add()" style="width: 60px; margin-right: 30px;">确定</a> <a class="mini-button" onclick="onCancel"
				style="width: 60px;  margin-right: 30px;">取消</a><a class="mini-button" onclick="help()"
				style="width: 60px;">帮助</a>
		</div>
	</form>
</body>
</html>