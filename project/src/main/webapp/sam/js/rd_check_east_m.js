...
function rdChange(e){ 
	...
	var rdOrg = mini.get("brNo").getValue();
	chooseMapOrg("RD",rdOrg,"EAST");
...
function onBalanceRenderer(e) {
	return "<span style=\"color: red\">" + (e.record.samValue - e.record.eastValue).toFixed(2) + "</span>"
}
...
function rowdblclick(e) {
	var record = e.record;
	mini.open({
		url : base + 'rd/formula/east/getFormulaById.nut?id=' + record.formulaId + '&page=/sam/jsp/rd_formula_east_look.jsp',
		title : '校验公式维护',
		width : 800,
		height : 500,
		allowResize : false,
...
function excute() {
	if (mini.get("brNo").getValue() == '' || mini.get("brNo").getValue() == null) {
		...
		$.ajax({
			type : 'POST',
			dataType : 'json',
			url : base + 'rd/check/east/startCheck.nut?rdBmcode=' + mini.get("brNo").getValue() + "&eastBmcode=" + mini.get('eastBmcode').getValue() + '&reportDate=' + mini.get("reportDate").getText(),
			success : function(text) {
				if (text.success) {
