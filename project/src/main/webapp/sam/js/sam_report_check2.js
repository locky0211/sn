$(function() {
			...
			$('#btnPrint').bind('click', function() {
						if ($('#checkResultDiv').text() != '') {
							myPrint('checkResultDivCon');
						}
					});
			$('#btnExportExcel').bind('click', function() {
				var url = base + '/sam/src/doReportCheck?' + "bankOrganNo=" + $('#bankOrganNoId').val() + "&checkDate=" + $('#reportDateId').val() + "&checkType=" + $('#checkTypeId').val()
						+ "&checkTypeName=" + $('#checkTypeId').find("option:selected").text() + "&bankOrganName=" + $('#bankOrganNoText').val();
				if ($('#checkResultDiv').text() != '') {
					window.open(encodeURI(url + "&isExportExcel=1"));
				}
			});
		});
function doReportCheck() {
	loadingShow("数据公式校验处理中...请稍等!!");
	$.ajax({
				type : "POST",
				dataType : 'json',
				url : base + '/sam/src2/doReportCheck',
				data : "bankOrganNo=" + $('#bankOrganNoId').val() + "&checkDate=" + $('#reportDateId').val() + "&checkType=" + $('#checkTypeId').val() + "&checkTypeName="
