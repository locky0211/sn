...
var swfobject = function() {
	var D = "undefined", r = "object", S = "Shockwave Flash", W = "ShockwaveFlash.ShockwaveFlash", q = "application/x-shockwave-flash", R = "SWFObjectExprInst", x = "onreadystatechange", O = window, j = document, t = navigator, T = false, U = [h], o = [], N = [], I = [], l, Q, E, B, J = false, a = false, n, G, m = true, M = function() {
		var aa = typeof j.getElementById != D && typeof j.getElementsByTagName != D && typeof j.createElement != D, ah = t.userAgent.toLowerCase(), Y = t.platform.toLowerCase(), ae = Y ? /win/
				.test(Y) : /win/.test(ah), ac = Y ? /mac/.test(Y) : /mac/.test(ah), af = /webkit/.test(ah) ? parseFloat(ah.replace(/^.*webkit\/(\d+(\.\d+)?).*$/, "$1")) : false, X = !+"\v1", ag = [0,
				0, 0], ab = null;
...
(function($) {
		...
		init : function(options, swfUploadOptions) {
			...
			return this.each(function() {
						...
						var settings = $.extend({
							...
							// Events
							overrideEvents : []
								// (Array) A list of default event handlers to
								// skip
								/*
								 * onCancel // Triggered when a file is
								 * cancelled from the queue onClearQueue //
								 * Triggered during the 'clear queue' method
								 * onDestroy // Triggered when the uploadify
								 * object is destroyed onDialogClose //
								 * Triggered when the browse dialog is closed
								 * onDialogOpen // Triggered when the browse
								 * dialog is opened onDisable // Triggered when
								 * the browse button gets disabled onEnable //
								 * Triggered when the browse button gets enabled
								 * onFallback // Triggered is Flash is not
								 * detected onInit // Triggered when Uploadify
								 * is initialized onQueueComplete // Triggered
								 * when all files in the queue have been
								 * uploaded onSelectError // Triggered when an
								 * error occurs while selecting a file (file
								 * size, queue size limit, etc.) onSelect //
								 * Triggered for each file that is selected
								 * onSWFReady // Triggered when the SWF button
								 * is loaded onUploadComplete // Triggered when
								 * a file upload completes (success or error)
								 * onUploadError // Triggered when a file upload
								 * returns an error onUploadSuccess // Triggered
								 * when a file is uploaded successfully
								 * onUploadProgress // Triggered every time a
								 * file progress is updated onUploadStart //
								 * Triggered immediately before a file upload
								 * starts
								 */
							}, options);
	...
	var handlers = {
		...
		onDialogOpen : function() {
			...
			this.queueData.errorMsg = '添加上传文件失败:';
		...
		onSelect : function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					var replaceQueueItem = confirm('文件 "' + file.name + '" 已进行过上传处理.\n是否现在进行替换?');
					if (!replaceQueueItem) {
				...
				switch (errorCode) {
					case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED :
						if (settings.queueSizeLimit > errorMsg) {
							...
							this.queueData.errorMsg += '最多支持上传 ' + settings.queueSizeLimit + ' 个文件!';
						}
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT :
						this.queueData.errorMsg += '文件: "' + file.name + '" 超过大小限制 (' + settings.fileSizeLimit + ').';
						break;
					case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE :
						this.queueData.errorMsg += '文件: "' + file.name + '" 是个空文件!!.';
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT :
						this.queueData.errorMsg += '文件 "' + file.name + '" 不是有效文件类型 (' + settings.fileTypeDesc + ').';
						break;
				}
			}
