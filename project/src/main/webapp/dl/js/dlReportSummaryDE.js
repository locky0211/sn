...
function add() {
	mini.open({
		url : base + 'dl/jsp/dlReportSummaryDEEdit.jsp',
		title : '新增汇总配置',
		width : 660,
		height : 400,
		allowResize : false,
...
function edit(e) {
	...
	mini.open({
		url : base + 'dl/reportSummary/getReportSummaryDEById.nut?id=' + e.id
				+ '&page=/dl/jsp/dlReportSummaryDEEdit.jsp',
		title : '汇总配置维护',
		width : 660,
		height : 400,
		allowResize : false,
		...
		mini.confirm("确定删除记录？", "确定？", function(action) {
			if (action == 'ok') {
				$.ajax({
					type : 'POST',
					dataType : 'json',
					url : base + 'dl/reportSummary/deleteReportSummaryDE.nut?id=' + e.id,
					success : function(text) {
						if (text) {
