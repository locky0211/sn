...
function toEditTableReport(tabCode, isReplay) {
	...
	mini.open({
				url : base + "ir/table/report/toIrTableReportForEdit.nut?tabCodeVersion=" + tabCode + "_" + isReplay + "&brNo=" + $('#organNo').val()
...
function toEditTableReportLastRate(tabCode, isReplay) {
	...
	mini.open({
				url : base + "ir/table/report/toIrTableReportForEditLastRate.nut?tabCodeVersion=" + tabCode + "_" + isReplay + "&brNo=" + $('#organNo').val()
						+ "&reportDate=" + $('#reportDate').val() + "&checkFormula=" + encodeURIComponent($('#formula').val()),
				title : tabCode + n,
				iconCls : "icon-excel",
				width : $(top.window).width() - 50,
