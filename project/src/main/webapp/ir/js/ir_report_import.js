...
function downLoadExcel(reportId) {
	...
	$.ajax({
			...
			url : base + 'ir/report/import/doExportExcel.nut',
			type : 'post',
			dataType : 'json',
			data : {
				reportId : reportId
...
function downLoadExcels() {
	...
	var rows = grid.getSelecteds();
            if (rows.length > 0) {
                ...
                toDownLoadFile(base + "ir/report/import/doExportExcelAll.nut?ids="+ids);
            } else {
                alert("请选中至少一条记录!");
            }
}
...
function toReportView(row_uid) {
	...
	mini.open({
				url : base + "ir/table/report/toIrTableReport.nut?reportId=" + row.reportId + "&tableId=" + row.tableId,
...
function doReportImport() {
	...
	if (form.isValid()) {
		if (mini.get('excelFile').getValue() == '' || mini.get('excelFilePath').getValue() == '') {
			...
			$.ajax({
						url : base + 'ir/report/import/doReportImport.nut',
						type : 'post',
						dataType : 'json',
						data : json,
						cache : false,
