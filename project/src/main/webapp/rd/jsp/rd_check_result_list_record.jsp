...
<title>历史校验结果列表</title>
<%@include file="../../common/quote/boot.html"%>
<script type="text/javascript" src="../js/rd_check_result_list_record.js"></script>
</head>
<body>
			...
			<table style="width: 100%;">
				<tr>
					<td style="width: 100%;"><label style="font-family: Verdana;"> 机构: </label> <input id="bmCode" showClose="true" oncloseclick="onCloseClick"
						...
						valueField="bmCode" resultAsTree="false" allowInput="false" onbeforenodeselect="beforenodeselect" value="${param.brNo }" />
						&nbsp;&nbsp;报表日期:&nbsp;<input id="rDate" name="reportDate" style="width: 80px;" class="mini-monthpicker" format="yyyyMM" required="true"
						value="${param.reportDate }" /> &nbsp;&nbsp;校验类型:&nbsp; <input id="tabType" name="tabType" class="mini-combobox" style="width: 80px;"
						textField="zdName" showNullItem="true" valueField="zdValue" value="${param.tabType }"
						url="${base }/sys/ggzd/getGgzdList.nut?groupId=RD_TABLE_TAB_TYPE" allowInput="false" />&nbsp;&nbsp;风险等级:&nbsp; <input id="checkRisk" name="checkRisk"
						class="mini-combobox" style="width: 80px;" textField="zdName" showNullItem="true" valueField="zdValue" value="${param.checkRisk }"
						url="${base }/sys/ggzd/getGgzdList.nut?groupId=RD_CHECK_RISK" allowInput="false" /> &nbsp;&nbsp; <a class="mini-button" iconCls="icon-search"
						onclick="doSearchCheckResult()">查找</a>&nbsp;&nbsp;&nbsp;<a class="mini-button" iconCls="icon-excel" onclick="doExportExcel()">导出Excel</a></td>
				</tr>
			</table>
		</div>
	</div>
	<div class="mini-fit">
		<div id="checkResultGrid" allowAlternating="true" class="mini-datagrid" onrowclick="showCheckView" style="width: 100%; height: 100%;"
			url="${base}/rd/check/getRdReportCheckResultListRecord.nut" pageSize="150" allowAlternating="true" showPager="true" showModified="false">
