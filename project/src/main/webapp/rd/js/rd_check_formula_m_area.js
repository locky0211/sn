...
function onRenderer1(e) {
	if (e.value == 'admin') {
		return '<font color="red">系统公式</font>';
...
function toUpdateCheckFormulaStatus(row_uid) {
	...
	$.ajax({
				type : 'POST',
				url : base + 'rd/check/updateCheckFormulaStatus.nut',
				data : {
					'id' : row.id,
...
function addCheckFormula(e) {
	mini.open({
				url : base + "rd/jsp/rd_check_formula.jsp?cUser=" + $('#cUserInput').val(),
				title : "新增校验公式",
				iconCls : "icon-add",
				width : 580,
				height : 450,
...
function edit() {
	...
	if (row) {
		mini.open({
					url : base + 'rd/check/toEditCheckFormula.nut?id=' + row.id + '&page=/rd/jsp/rd_check_formula.jsp',
					title : '校验公式修改',
					iconCls : 'icon-edit',
					width : 580,
					height : 450,
	...
	if (row) {
		mini.confirm('确定删除？删除后无法恢复!', '确定？', function(action) {
					if (action == 'ok') {
						$.ajax({
									type : 'POST',
									url : base + 'rd/check/deleteCheckFormula.nut',
									data : {
										'id' : row.id
									},
									dataType : 'json',
