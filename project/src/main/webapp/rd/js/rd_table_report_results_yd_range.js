...
function addCss(){
	$.ajax({
		...
		dataType : 'json',
		async : false,
		url : base + 'importWaveDesc/getCellLocal.nut?tableId='+mini.get('tableId').getValue()
		+'&brNo='+mini.get('brNo').getValue()+'&reportDate='+mini.get('reportDate').getValue()
		+'&checkType='+mini.get('checkType').getValue()+'&cUser='+mini.get('cUser').getValue(),
		success : function(data) {
			cellLocal = data;
			if (cellLocal != '') {
				$.each(cellLocal.split(';'), function(i, n) {
					var list=n.split(',');
					if(list[1]=='1'){
						$('#' + list[0] + '_TD').addClass('acceptInput');
					}else{
						$('#' + list[0] + '_TD').addClass('errorInput');
					}
				});
			}
		}
...
function AlertTab() {
	...
	mini.open({
		url : base + "rd/check/wave/toRdReportCheckResultsView.nut?id=" + formulaId + '&page=/rd/jsp/rd_report_check_results_view_wave.jsp',
		title : "校验结果",
		iconCls : "icon-text",
		width : 750,
		height : 440,
