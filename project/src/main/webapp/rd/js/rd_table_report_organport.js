...
function doSaveReportDate() {
	if ($('#isChanged').val() == 'y') {
		var url = base + 'rd/table/report/doSaveTableReportData.nut';
		mini.mask({
					el : document.body,
...
function doRepaceReportData() {
	mini.confirm('是否确定覆盖为导入前数据状态?!', '确定？', function(action) {
				if (action == 'ok') {
					$.ajax({
								type : 'POST',
								url : base + "rd/report/import/doReplaceReportDate.nut",
								data : {
...
function doTableReportImport() {
	mini.open({
				url : base + 'rd/jsp/rd_table_report_import.jsp?reportId=' + $('#reportId').val(),
				title : '数据导入',
				width : 366,
				height : 198,
				allowResize : false,
...
function changeValue(){
	...
	if(reg.test(tdtxt.replace(/,/g,""))){
		mini.open({
			url : base + 'rd/jsp/rd_table_report_changeValue_organport.jsp?&value='+tdtxt+'&inputId='+inputId+''+'&reportId='+reportId+'&reportDate='+reportDate,
			title : '数据修改',
			width : 300,
			height : 140,
			allowResize : false,
