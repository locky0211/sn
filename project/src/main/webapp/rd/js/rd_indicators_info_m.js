...
function saveDataRowSort() {
	...
	if (rows.length > 0) {
		...
		$.ajax({
					type : "POST",
					url : base + "rd/indicators/doSaveIndSort.nut",
...
function edit() {
	...
	if (uId == row.cUser) {
		if (row) {
			mini.open({
						url : base + 'rd/jsp/rd_indicators_info_list.jsp?pId=' + row.id,
						title : '指标信息',
						iconCls : 'icon-33',
						width : 680,
						height : 380,
...
function toUpdateindicatorsInfoStatus(row_uid) {
	...
	$.ajax({
				type : 'POST',
				url : base + 'rd/indicators/updateIndicatorsBasicInfoValidFlag.nut',
				data : {
					'id' : row.id,
...
function addIndicatorsBasicInfo() {
	mini.open({
				url : base + "rd/jsp/rd_indicators_basic_info.jsp?isParent=y&iconCls=icon-statistics",
				title : "新增指标类别",
				iconCls : "icon-add",
				width : 400,
				height : 420,
...
function addIndicatorsInfo(e) {
	mini.open({
				url : base + "rd/jsp/rd_indicators_basic_info.jsp?isParent=n&iconCls=icon-36",
				title : "新增指标信息",
				iconCls : "icon-add",
				width : 520,
				height : 400,
...
function updateRow(row) {
	$.ajax({
				type : 'POST',
				url : base + 'rd/indicators/fetchindicatorsInfo.nut',
				data : {
					'id' : row.id
				},
				dataType : 'json',
...
function update() {
	...
	if (uId == row.cUser) {
		if (row) {
			if (row.isParent == 'y') {
				mini.open({
							url : base + 'rd/indicators/toEditIndicatorsBasicInfo.nut?id=' + row.id
									+ '&page=/rd/jsp/rd_indicators_basic_info.jsp?isParent=y&iconCls=icon-statistics',
							title : '指标类型修改',
							iconCls : 'icon-edit',
							width : 400,
							height : 420,
				...
				mini.open({
							url : base + 'rd/indicators/toEditIndicatorsBasicInfo.nut?id=' + row.id
									+ '&page=/rd/jsp/rd_indicators_basic_info.jsp?isParent=n&iconCls=icon-36',
							title : '指标信息修改',
							iconCls : 'icon-edit',
							width : 520,
							height : 400,
		...
		if (row) {
			mini.confirm('删除后可能影响系统正常运行!', '确定？', function(action) {
						if (action == 'ok') {
							$.ajax({
										type : 'POST',
										url : base + 'rd/indicators/deleteIndicatorsBasicInfo.nut',
										data : {
											'id' : row.id
										},
										dataType : 'json',
