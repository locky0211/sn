...
function toEditTableReport(tabCode, isReplay) {
	mini.open({
				url : base + "rd/table/report/toRdTableReportForEdit_OrganPort.nut?tabCodeVersion=" + tabCode + "_" + isReplay + "&brNo=" + $('#organNo').val()
...
function toEditTableReportLastRate(tabCode, isReplay) {
	var n = '上期';
	mini.open({
				url : base + "rd/table/report/toRdTableReportForEditLastRate_OrganPort.nut?tabCodeVersion=" + tabCode + "_" + isReplay + "&brNo=" + $('#organNo').val()
...
function downIssue() {
	...
	var oldContent=mini.get('oldContent').getValue();
	...
	$.ajax({
		url : base + 'rd/check/wave_organport/saveRemark_OrganPort.nut',
		type : 'post',
		dataType : 'json',
		data : {
