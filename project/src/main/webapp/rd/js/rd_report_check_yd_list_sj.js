	...
	form.validate();
		if (reportDateValid()) {
			mini.mask({
			 ...
			 $.ajax({
			        url: base+'rd/check/wave/toMakeFormula.nut?reportDate='+reportDate+'&reportRate='+reportRate+'&tabType='+tabType,
			        type: "post",
			        success: function (text) {
			        	if(text.success){
			        		mini.alert("操作成功!!");
...
function showCheckView(e) {
	var row = e.record;
	mini.open({
				url : base + "rd/check/wave/toRdReportCheckResultsViewCS.nut?id=" + row.id + '&page=/rd/jsp/rd_report_check_results_view_wave_sj.jsp',
				title : "校验结果",
				iconCls : "icon-text",
				width : 750,
				height : 400,
