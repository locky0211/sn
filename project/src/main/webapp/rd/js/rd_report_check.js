    ...
    if (rows.length > 0) {
		...
		if (form.isValid()) {
			if(reportDateValid()){
				...
				$.ajax({
							url : base + 'rd/check/wave/toReportCheckForYD.nut?brNo='+brNo+'&tabCodes='+tabCode+'&reportDate='+reportDate+'&checkType='+checkType+'&nodeLe='+nl,
							type : 'post',
							cache : false,
		...
		if (form.isValid()) {
			if(reportDateValid()){
				...
				$.each(codes, function(i, n) {
					$.ajax({
								url : base + 'rd/check/wave/doExportExcelYD.nut?tabCodes=' + n + '&brNo=' + brNo + '&reportDate=' + reportDate + '&checkType=' + checkType +'&isValue='+isValue,
								cache : false,
								success : function(text) {
									if (text != null) {
...
function toReportView(reportNoStr,isdValue) {
	...
	if(reportDate!=null&&reportDate!=""){
		mini.open({
					url : base + "rd/check/wave/getCheckRusultsForYD.nut?tabCode=" + reportNoStr + "&checkType=" + checkType + "&reportDate=" + reportDate +"&brNo=" + brNo+'&isdValue='+isdValue,
					title : "阈值明细",
					iconCls : "icon-excel",
