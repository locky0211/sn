...
function downLoadExcel(reportId) {
	...
	//机构
	$.ajax({
		url : base + 'rd/report/import_cs/checkDoExport.nut',
		type : 'post',
		dataType : 'json',
		data : {
			reportId : reportId
...
function toReportView(row_uid) {
	...
	mini.open({
				url : base + "rd/table/report/toRdTableReportCs.nut?reportId=" + row.reportId + "&tableId=" + row.tableId,
...
function doReportImport() {
	...
	if (form.isValid()) {
		if (mini.get('excelFile').getValue() == '' || mini.get('excelFilePath').getValue() == '') {
			...
			$.ajax({
						url : base + 'rd/report/import_cs/doReportImport.nut',
						type : 'post',
						dataType : 'json',
						data : json,
						cache : false,
