...
function doExportIndDataExcel() {
	...
	if (form.isValid() && checkDate()) {
		...
		$.ajax({
					url : base + 'rd/ind/data/report/doExportIndDataExcel1.nut',
					type : 'post',
					dataType : 'json',
					data : {
...
function doInItDataExcel() {
	...
	$.ajax({
				url : base + 'rd/ind/data/report/initIndDataExcel1.nut',
				type : 'post',
				dataType : 'json',
				cache : false,
				data : {
...
function doInItDataPie() {
	...
	$.ajax({
				url : base + 'rd/ind/data/report/initIndDataReport1.nut',
				type : 'post',
				dataType : 'json',
				cache : false,
				data : {
...
function initRdIndList() {
	...
	if (brNo != '' && indType != '') {
		mini.get('inds').load(base + 'rd/ind/data/getRdIndLists.nut?brNo=' + brNo + '&indType=' + indType);
	}
}
