...
function addRow(e) {
	mini.open({
		url : base
				+ "rd/dm/toAddDataTaskList.nut?page=/rd/jsp/dm_data_task_list.jsp",
		title : "新增处理任务",
		iconCls : "icon-add",
		width : 575,
		height : 520,
...
function toUpdateTaskFlag(row_uid) {
	...
	$.ajax({
				type : 'POST',
				url : base + 'rd/dm/updateDataTaskFlag.nut',
				data : {
					'id' : row.id,
					'flag' : row.taskFlag
				},
...
function edit(row_uid) {
	...
	if (row) {
		mini.open({
					url : base + 'rd/dm/toEditDataTask.nut?id=' + row.id
							+ '&page=/rd/jsp/dm_data_task_list.jsp',
					title : '调度处理任务',
					iconCls : 'icon-edit',
					width : 575,
					height : 520,
	...
	if (row) {
		mini.confirm('删除后可能影响数据处理正常运行!', '确定？', function(action) {
					if (action == 'ok') {
						$.ajax({
									type : 'POST',
									url : base + 'rd/dm/deleteDataTask.nut',
									data : {
										'id' : row.id
									},
									dataType : 'json',
