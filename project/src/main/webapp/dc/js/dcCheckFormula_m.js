...
function add() {
	mini.open({
		url : base + 'dc/jsp/dcCheckFormula.jsp',
		title : '校验公式维护',
		width : 660,
		height : 400,
		allowResize : false,
...
function edit(e) {
	...
	mini.open({
		url : base + 'dcFormula/getDcFormulaById.nut?id=' + e.id + '&page=/dc/jsp/dcCheckFormula.jsp',
		title : '客户名称维护',
		width : 660,
		height : 400,
		allowResize : false,
...
function chageState() {
	...
	$.ajax({
		type : 'POST',
		dataType : 'json',
		url : base + 'dcFormula/updateDcFormulaState.nut?id=' + e.id,
		success : function(text) {
			if (text.success) {
		...
		mini.confirm("确定删除记录？", "确定？", function(action) {
			if (action == 'ok') {
				$.ajax({
					type : 'POST',
					dataType : 'json',
					url : base + 'dcFormula/deleteDcFormula.nut?id=' + e.id,
					success : function(text) {
						if (text) {
