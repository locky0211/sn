...
function add() {
	mini.open({
		url : base + 'dc/jsp/dc_url.jsp?flag=1',
		title : '数据库信息配置',
		width : 450,
		height : 350,
		allowResize : false,
...
function edit(e) {
	...
	mini.open({
		url : base + 'dc/url/getDcUrlPageByNo.nut?no=' + e.brNo + '&page=/dc/jsp/dc_url.jsp?flag=2',
		title : 'EAST数据库信息配置',
		width : 450,
		height : 350,
		allowResize : false,
		...
		mini.confirm("确定删除记录？", "确定？", function(action) {
			if (action == 'ok') {
				$.ajax({
					type : 'POST',
					dataType : 'json',
					url : base + 'dc/url/deleteDcUrl.nut?brNo=' + e.brNo,
