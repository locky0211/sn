...
function doExportExcelSearch() {
	...
	if (form.isValid()) {
		...
		$.ajax({
					url : base + 'pi/check/wave/doExportExcelSearch.nut',
					type : 'post',
					dataType : 'json',
					data : {
...
function showCheckView(e) {
	var row = e.record;
	mini.open({
				url : base + "pi/check/wave/toBfReportCheckResultsViewYD.nut?id=" + row.id + '&page=/pi/jsp/pi_report_check_results_view_wave.jsp',
				title : "校验结果",
				iconCls : "icon-text",
				width : 750,
				height : 400,
...
function doReportCheck() {
	...
	if (form.isValid()) {
		if (reportDateValid()) {
			...
			$.ajax({
						url : base + 'pi/check/wave/toReportCheck.nut?brNo='+brNo+'&reportDate='+reportDate+'&reportRate='+reportRate+'&tabType='+tabType+'&nodeLe='+nl,
						type : 'post',
						cache : false,
						success : function(text) {
							if (nl > 1) {
...
function saveRemarks(){
	...
	mini.mask({
	 ...
	 $.ajax({
	        url: base+"pi/check/wave/saveRemark.nut",
	        data: { data: json },
	        type: "post",
	        success: function (text) {
	        	if(text.success){
