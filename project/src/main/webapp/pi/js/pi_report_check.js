	...
	if (form.isValid()) {
		if (reportDateValid()) {
			 var nodes = mini.get('bmCode').getCheckedNodes(false);
				...
				$.ajax({
							url : base + 'pi/check/toReportCheck.nut',
							type : 'post',
							dataType : 'json',
							data : {
...
function doExportExcel() {
	...
	if (form.isValid()) {
		...
		$.ajax({
					url : base + 'pi/check/doExportExcel.nut',
					type : 'post',
					dataType : 'json',
					data : {
						brNo : mini.get('bmCode').getValue(false),
...
function showCheckView(e) {
	var row = e.record;
	mini.open({
				url : base + "pi/check/toPiReportCheckResultsView.nut?id=" + row.id + '&page=/pi/jsp/pi_report_check_results_view.jsp',
				title : "校验结果",
				iconCls : "icon-text",
				width : 750,
				height : 400,
