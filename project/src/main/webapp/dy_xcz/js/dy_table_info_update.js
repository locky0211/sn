...
function edit(row_uid) {
	...
	if (row) {
		mini.open({
					url : base + 'dy_xcz/table/toEditTableInfo.nut?tabCode=' + row.tabCode+ '&versionNo=' + row.versionNo 
							+ '&page=/dy_xcz/jsp/dy_table_info.jsp',
					title : '报表模板信息修改',
					iconCls : 'icon-edit',
					width : 550,
					height : 550,
	...
	if (row) {
		mini.confirm('删除后可能影响系统正常运行!', '确定？', function(action) {
					if (action == 'ok') {
						$.ajax({
									type : 'POST',
									url : base + 'dy_xcz/table/deleteTableInfo.nut',
									data : {
										'tableId' : row.tableId
									},
									dataType : 'json',
...
function expo(row_uid){
	...
	$.ajax({
			url : base + 'dy_xcz/table/doExportExcel.nut',
			type : 'post',
			dataType : 'json',
			data : {
				'tableId' : row.tableId
