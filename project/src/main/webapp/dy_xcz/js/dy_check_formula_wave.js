function add() {
	...
	if (form.isValid()) {
		...
		$.ajax({
					url : base + 'dy_xcz/check/wave/addOrUpdateCheckFormula.nut',
					type : 'post',
					data : json,
					dataType : 'json',
					cache : false,
...
function onDeptChanged(e) {
	...
	var url = "../../dy_xcz/table/getTableVersionNo.nut?tabCode="+tabcode;
	mini.get("thisVersion").setUrl(url);
	mini.get("lastVersion").setUrl(url);
}
function help(){
	var schemaType='DY_XCZ.DY_XCZ_CHECK_FORMULA';
	mini.open({
		url : base + "sys/help/toShowSysHelp.nut?schemaType=" + schemaType + '&page=/dy_xcz/jsp/dy_table_info_help.jsp',
		title : '帮助',
		iconCls : 'icon-edit',
		width : 850,
		height : 350,
