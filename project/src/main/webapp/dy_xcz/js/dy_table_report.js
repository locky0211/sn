$(function() {
			...
			if ($('#bjFlag').val() == 'true') {
				$('.td-data_1').live('mouseover', function() {
...
function doSaveReportDate() {
	if ($('#isChanged').val() == 'y') {
		var url = base + 'dy_xcz/table/report/doSaveTableReportData.nut';
		mini.mask({
					el : document.body,
...
function doRepaceReportData() {
	mini.confirm('是否确定覆盖为导入前数据状态?!', '确定？', function(action) {
				if (action == 'ok') {
					$.ajax({
								type : 'POST',
								url : base + "dy_xcz/report/import/doReplaceReportDate.nut",
								data : {
...
function doExportExcel() {
	var url = base + 'dy_xcz/table/report/toExportTableReportExcel.nut?reportId=' + $('#reportId').val();
	mini.mask({
				el : document.body,
				cls : 'mini-mask-loading',
				html : '生成Excel文件中...'
...
function doTableReportImport() {
	mini.open({
				url : base + 'dy_xcz/jsp/dy_table_report_import.jsp?reportId=' + $('#reportId').val(),
				title : '数据导入',
				width : 366,
				height : 198,
				allowResize : false,
	...
	for(var k=0;k<document.getElementsByTagName("table").length;k++){
		...
		for(var i=0;i<tbmain.rows.length;i++){ 
			for(var j=0;j<tbmain.rows[i].cells.length;j++){ 
				tbmain.rows[i].cells[j].onmouseover=changeValue; 
			} 
		}
	}
	var inputId=document.getElementsByTagName("input");
	inputId.onmouseover=changeValue;
} 
...
function changeValue(){
	...
	if(reg.test(tdtxt.replace(/,/g,""))){
		mini.open({
			url : base + 'dy_xcz/jsp/dy_table_report_changeValue.jsp?&value='+tdtxt+'&inputId='+inputId+''+'&reportId='+reportId+'&reportDate='+reportDate,
			title : '数据修改',
			width : 300,
			height : 140,
			allowResize : false,
