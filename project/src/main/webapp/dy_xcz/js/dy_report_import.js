...
$(function() {
			...
			$.ajax({
				...
				dataType : 'json',
				async : false,
				url : base + 'sys/ggzd/getGgzdList.nut?groupId=DY_TABLE_TAB_TYPE',
...
function downLoadExcel(reportId) {
	...
	$.ajax({
			...
			url : base + 'dy_xcz/report/import/doExportExcel.nut',
			type : 'post',
			dataType : 'json',
			data : {
				reportId : reportId
...
function downLoadExcels() {
	...
	var rows = grid.getSelecteds();
            if (rows.length > 0) {
                ...
                toDownLoadFile(base + "dy_xcz/report/import/doExportExcelAll.nut?ids="+ids);
            } else {
                alert("请选中至少一条记录!");
            }
}
...
function toReportView(row_uid) {
	...
	mini.open({
				url : base + "dy_xcz/table/report/toDyTableReport.nut?reportId=" + row.reportId + "&tableId=" + row.tableId,
...
function doReportImport() {
	...
	if (form.isValid()) {
		if (mini.get('excelFile').getValue() == '' || mini.get('excelFilePath').getValue() == '') {
			...
			$.ajax({
						url : base + 'dy_xcz/report/import/doReportImport.nut',
						type : 'post',
						dataType : 'json',
						data : json,
						cache : false,
...
function doReportExport() {
	...
	if (dataStr.length != 0) {
		...
		$.ajax({
					url : base + 'dy_xcz/table/report/toExportTableReportBatchExcel.nut?reportIds=' + dataStr.substring(0, dataStr.length - 1)
