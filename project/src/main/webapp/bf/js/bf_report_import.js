...
function clearReport(reportId){
	mini.confirm('确定清空？清空后无法恢复!', '确定？', function(action) {
		if (action == 'ok') {
			$.ajax({
						type : 'POST',
						url : base + 'bf/report/import/clearReport.nut',
						data : {
							reportId : reportId
						},
						dataType : 'json',
...
function downLoadExcel(reportId) {
	...
	$.ajax({
			...
			url : base + 'bf/report/import/doExportExcel.nut',
			type : 'post',
			dataType : 'json',
			data : {
				reportId : reportId
...
function downLoadExcels() {
	...
	var rows = grid.getSelecteds();
            if (rows.length > 0) {
                ...
                toDownLoadFile(base + "bf/report/import/doExportExcelAll.nut?ids="+ids);
            } else {
                alert("请选中至少一条记录!");
            }
}
...
function toReportView(row_uid) {
	...
	mini.open({
				url : base + "bf/table/report/toRdTableReport.nut?reportId=" + row.reportId + "&tableId=" + row.tableId,
