...
function showCheckView(e) {
	...
	if(row.remarks=='校验通过,提交复核'){
	 ...
	 $.ajax({
	        url: base+"bf/remarks/temp/hasRemarksRecord.nut",
	        data: { id: row.id },
	        type: "post",
	        success: function (text) {
	        	if(text.success){
	        		mini.open({
	    				url : base + "bf/remarks/temp/toBfReportRemarksResultsView.nut?id=" + row.id + '&page=/bf/jsp/bf_report_check_results_view_r_t.jsp',
	    				title : "校验结果",
	    				iconCls : "icon-text",
	    				width : 750,
	    				height : 400,
...
function reviewSuccess(){
	...
	if (rows.length > 0) {
		...
		$.ajax({
			type : "POST",
			url : base + "bf/remarks/temp/reviewRemarks.nut",
			data : {"ids" : ids},
			dataType : 'json',
...
function onlyUpdateRemark(){
	 var grid=mini.get('remarksResultGrid');
		...
		grid.loading("提交中请稍后。。。");
		 $.ajax({
		        url: base+"bf/remarks/temp/onlyUpdateRemarks.nut",
		        data: { data: json },
		        type: "post",
		        success: function (text) {
		        	if(text.success){
...
function deleteRemark(){
	...
	if (rows.length > 0) {
		...
		$.ajax({
			type : "POST",
			url : base + "bf/remarks/temp/deleteRemarks.nut",
			data : {"ids" : ids},
			dataType : 'json',
			success : function(data) {
				if (data.success) {
...
function submitRemarks(){
	 var grid=mini.get('remarksResultGrid');
		...
		grid.loading("提交中请稍后。。。");
		 $.ajax({
		        url: base+"bf/remarks/temp/updateRemarks.nut",
		        data: { data: json },
		        type: "post",
		        success: function (text) {
		        	if(text.success){
...
function submitRemarksTH(){
	 var grid=mini.get('remarksResultGrid');
		...
		grid.loading("提交中请稍后。。。");
		 $.ajax({
		        url: base+"bf/remarks/temp/updateRemarksTH.nut",
		        data: { data: json },
		        type: "post",
		        success: function (text) {
		        	if(text.success){
...
function onBrNo(e){
	...
	$.ajax({
        url: base+"bf/remarks/temp/getBmNameByBmCode.nut",
        data: { code:brNo },
        type: "post",
        async:false,
        success: function (text) {
...
function cancelRemark(){
	...
	$.ajax({
		type : "POST",
		url : base + "bf/remarks/temp/cancelRemarks.nut",
		data : {"ids" : ids},
		dataType : 'json',
		success : function(data) {
			if (data.success) {
...
function agreeNCancel(){
	...
	$.ajax({
		type : "POST",
		url : base + "bf/remarks/temp/agreeNCancel.nut",
		data : {"ids" : ids},
		dataType : 'json',
		success : function(data) {
			if (data.success) {
...
function agreeCancel(){
	...
	$.ajax({
		type : "POST",
		url : base + "bf/remarks/temp/agreeCancel.nut",
		data : {"ids" : ids},
		dataType : 'json',
		success : function(data) {
			if (data.success) {
