...
function toUpdateCheckFormulaStatus(row_uid) {
	...
	$.ajax({
				type : 'POST',
				url : base + 'bf/check/wave/updateCheckFormulaStatus.nut',
				data : {
					id : row.id,
...
function addCheckFormula(e) {
	mini.open({
				url : base + "bf/jsp/bf_check_formula_wave.jsp",
				title : "新增校验公式",
				iconCls : "icon-add",
				width : 520,
				height : 450,
...
function edit() {
	...
	if (row) {
		mini.open({
					url : base + 'bf/check/wave/toEditCheckFormulaWave.nut?id=' + row.id + '&page=/bf/jsp/bf_check_formula_wave.jsp',
					title : '校验公式修改',
					iconCls : 'icon-edit',
					width : 520,
					height : 450,
...
function updateRow(row) {
	$.ajax({
				type : 'POST',
				url : base + 'bf/check/wave/fetchCheckFormulaWave.nut',
				data : {
					'id' : row.id
				},
				dataType : 'json',
	...
	if (row) {
		mini.confirm('确定删除？删除后无法恢复!', '确定？', function(action) {
					if (action == 'ok') {
						$.ajax({
									type : 'POST',
									url : base + 'bf/check/wave/deleteCheckFormulaWave.nut',
									data : {
										'id' : row.id
									},
									dataType : 'json',
...
function makeFormula() {
	...
	if (form.isValid()) {
		if (reportDateValid()) {
			...
			$.ajax({
				url : base + 'bf/wave/formula/makeCheckWaveFormula.nut?brNo='
