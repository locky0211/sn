...
function downLoadExcel(reportId) {
	...
	$.ajax({
				url : base + 'bf/report/Summary/doExportExcel.nut',
				type : 'post',
				dataType : 'json',
				data : {
					reportId : reportId,
...
function toReportView(row_uid) {
	...
	mini.open({
				url : base + "bf/table/report/toRdTableReport.nut?reportId=" + row.reportId + "&tableId=" + row.tableId+"&reportDate="+reportDate,
...
function doReportSummary() {
	...
	if (form.isValid()) {
			...
			$.ajax({
				url : base + 'bf/report/Summary/bfReportSummary.nut',
				type : 'post',
				dataType : 'json',
				data : json,
				cache : false,
...
function doPartTask() {
	...
	mini.open({
		url : base + "bf/jsp/bf_report_summary_taskList.jsp?&brNo="+brNo+"&reportDate="+reportDate+"&tabType="+tabType+"&xnjg="+xnjg,
		title : '查看执行步骤',
		iconCls : 'icon-edit',
		width : 800,
		height : 500,
...
function doBatchSummary(){
	 ...
	 if(reportDate==null||reportDate==''||tabType==null||tabType==''){
		 ...
		 $.ajax({
				url : base + 'bf/report/Summary/bfReportBatchSummary.nut',
				type : 'post',
				dataType : 'json',
