	...
	if (form.isValid()) {
		if (reportDateValid()) {
			 var nodes = mini.get('bmCode').getCheckedNodes(false);
				...
				$.ajax({
							url : base + 'bf/check/toReportCheck.nut',
							type : 'post',
							dataType : 'json',
							data : {
...
function onGridLoad(obj) {
	...
	$.ajax({
        url: base+"bf/remarks/checkVia.nut",
...
function showCheckView(e) {
	var row = e.record;
	mini.open({
				url : base + "bf/check/toBfReportCheckResultsView.nut?id=" + row.id + '&page=/bf/jsp/bf_report_check_results_view.jsp',
				title : "校验结果",
				iconCls : "icon-text",
				width : 750,
				height : 400,
...
function saveRemarks(){
   ...
   if(!grid.isChanged()){
	...
	grid.loading("提交中请稍后。。。");
	 $.ajax({
	        url: base+"bf/remarks/saveRemark.nut",
	        data: { data: json },
	        type: "post",
	        success: function (text) {
	        	if(text.success){
...
function reviewt(){
    ...
    tabType = mini.get("tabType").getValue();
	 $.ajax({
	        url: base+"bf/remarks/submitReview.nut",
...
function otherReviewt(){
    ...
    var tabType = mini.get("tabType").getValue();
	$.ajax({
		url: base+"bf/remarks/submitOtherReview.nut",
	    data: { brNo: message,reportDate:reportDate,tabType:tabType },
	    type: "post",
	    success: function (text) {
	    	if(text.success){
