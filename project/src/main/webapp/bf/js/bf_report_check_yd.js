...
function showCheckView(e) {
	var row = e.record;
	mini.open({
				url : base + "bf/check/wave/toBfReportCheckResultsViewYD.nut?id=" + row.id + '&page=/bf/jsp/bf_report_check_results_view_wave.jsp',
				title : "校验结果",
				iconCls : "icon-text",
				width : 750,
				height : 400,
...
function doReportCheck() {
	...
	if (form.isValid()) {
		if (reportDateValid()) {
			...
			$.ajax({
						url : base + 'bf/check/wave/toReportCheck.nut?brNo='+brNo+'&reportDate='+reportDate+'&reportRate='+reportRate+'&tabType='+tabType+'&nodeLe='+nl,
						type : 'post',
						cache : false,
						success : function(text) {
							if (nl > 1) {
