...
function toUpdateCheckFormulaStatus(row_uid) {
	...
	$.ajax({
				type : 'POST',
				url : base + 'bf/report/updateCheckFormulaStatus.nut',
				data : {
					'id' : row.id,
...
function addCheckFormula(e) {
	mini.open({
				url : base + "bf/jsp/bf_report_formula.jsp?cUser=" + $('#cUserInput').val(),
				title : "新增校验公式",
				iconCls : "icon-add",
				width : 580,
				height : 450,
...
function edit() {
	...
	if (row) {
		mini.open({
					url : base + 'bf/report/toEditCheckFormula.nut?id=' + row.id + '&page=/bf/jsp/bf_report_formula.jsp',
					title : '校验公式修改',
					iconCls : 'icon-edit',
					width : 580,
					height : 450,
...
function updateRow(row) {
	$.ajax({
				type : 'POST',
				url : base + 'bf/report/fetchCheckFormula.nut',
				data : {
					'id' : row.id
				},
				dataType : 'json',
	...
	if (row) {
		mini.confirm('确定删除？删除后无法恢复!', '确定？', function(action) {
					if (action == 'ok') {
						$.ajax({
									type : 'POST',
									url : base + 'bf/report/deleteCheckFormula.nut',
									data : {
										'id' : row.id
									},
									dataType : 'json',
