...
function toUpdateCheckFormulaStatus(row_uid) {
	...
	$.ajax({
				type : 'POST',
				url : base + 'bf/withRd/updateCheckFormulaStatus.nut',
				data : {
					'id' : row.id,
...
function addCheckFormula(e) {
	mini.open({
				url : base + "bf/jsp/bf_check_withrd_formula.jsp",
				title : "新增校验公式",
				iconCls : "icon-add",
				width : 580,
				height : 450,
...
function edit() {
	...
	if (row) {
		mini.open({
					url : base + 'bf/withRd/toEditCheckFormula.nut?id=' + row.id + '&page=/bf/jsp/bf_check_withrd_formula.jsp',
					title : '校验公式修改',
					iconCls : 'icon-edit',
					width : 580,
					height : 450,
...
function updateRow(row) {
	$.ajax({
				type : 'POST',
				url : base + 'bf/withRd/fetchCheckFormula.nut',
				data : {
					'id' : row.id
				},
				dataType : 'json',
	...
	if (row) {
		mini.confirm('确定删除？删除后无法恢复!', '确定？', function(action) {
					if (action == 'ok') {
						$.ajax({
									type : 'POST',
									url : base + 'bf/withRd/deleteCheckFormula.nut',
									data : {
										'id' : row.id
									},
									dataType : 'json',
