...
function add() {
	...
	if (form.isValid() && checkDeviationValue()) {
		...
		$.ajax({
					url : base + 'bf/withRd/addOrUpdateFormula.nut',
					type : 'post',
					data : json,
					dataType : 'json',
					cache : false,
...
function checkIsOnly(e) {
	...
	if (regu.test(e.value)) {
		...
		// 唯一性校验
		$.ajax({
					url : base + "bf/withRd/checkIsOnly.nut?checkFormula=" + e.value + "&id=" + mini.get("id").getValue(),
					async : false,
					dataType : 'json',
...
//	}
function help(){
	var schemaType='RD_WITH_BF_CHECK_FORMULA';
	mini.open({
