...
function doExportExcel() {
	var url = base + 'bf/table/report/toExportTableReportExcel.nut?reportId='
			+ $('#reportId').val();
	mini.mask({
		el : document.body,
		cls : 'mini-mask-loading',
...
function doTableReportImport() {
	mini.open({
		url : base + 'bf/jsp/rd_table_report_import.jsp?reportId='
				+ $('#reportId').val(),
		title : '数据导入',
		width : 366,
		height : 198,
...
function changeValue() {
	...
	if (reg.test(tdtxt.replace(/,/g, ""))) {
		mini.open({
			url : base + 'bf/jsp/bf_table_report_changeValue.jsp?&value='
...
function show() {
	...
	if (reg.test(tdtxt)) {
		$.ajax({
			type : 'POST',
			url : base + 'bf/table/report/showIndExplain.nut?value=' + tdtxt
					+ '&inputId=' + inputId,
			dataType : 'json',
			success : function(text) {
				if (text.data != null) {
