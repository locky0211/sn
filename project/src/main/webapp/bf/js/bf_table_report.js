...
function changeValue(){
	...
	if(reg.test(tdtxt.replace(/,/g,""))){
		mini.open({
			url : base + 'bf/jsp/bf_table_report_changeValue.jsp?&value='+tdtxt+'&inputId='+inputId+''+'&reportId='+reportId+'&reportDate='+reportDate,
			title : '数据修改',
			width : 300,
			height : 140,
			allowResize : false,
...
function show(){
	...
	if(reg.test(tdtxt)){
		$.ajax({
			type : 'POST',
			url : base + 'bf/table/report/showIndExplain.nut?value='+tdtxt+'&inputId='+inputId,
			dataType : 'json',
			success : function(text) {
				if(text.data!=null){
