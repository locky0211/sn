...
function showCheckView(e) {
	...
	if(row.remarks=='校验通过,提交复核'){
	 ...
	 $.ajax({
	        url: base+"bf/remarks/hasRemarksRecord.nut",
	        data: { id: row.id },
	        type: "post",
	        success: function (text) {
	        	if(text.success){
	        		mini.open({
	    				url : base + "bf/remarks/toBfReportRemarksResultsView.nut?id=" + row.id + '&page=/bf/jsp/bf_report_check_results_view_r.jsp',
	    				title : "校验结果",
	    				iconCls : "icon-text",
	    				width : 750,
	    				height : 400,
...
function reviewSuccess(){
	...
	if (rows.length > 0) {
		...
		$.ajax({
			type : "POST",
			url : base + "bf/remarks/reviewRemarks.nut",
			data : {"ids" : ids},
			dataType : 'json',
...
function onlyUpdateRemark(){
	 var grid=mini.get('remarksResultGrid');
		...
		grid.loading("提交中请稍后。。。");
		 $.ajax({
		        url: base+"bf/remarks/onlyUpdateRemarks.nut",
		        data: { data: json },
		        type: "post",
		        success: function (text) {
		        	if(text.success){
...
function deleteRemark(){
	...
	if (rows.length > 0) {
		...
		$.ajax({
			type : "POST",
			url : base + "bf/remarks/deleteRemarks.nut",
			data : {"ids" : ids},
			dataType : 'json',
			success : function(data) {
				if (data.success) {
...
function submitRemarks(){
	 var grid=mini.get('remarksResultGrid');
		...
		grid.loading("提交中请稍后。。。");
		 $.ajax({
		        url: base+"bf/remarks/updateRemarks.nut",
		        data: { data: json },
		        type: "post",
		        success: function (text) {
		        	if(text.success){
...
function submitRemarksTH(){
	 var grid=mini.get('remarksResultGrid');
		...
		grid.loading("提交中请稍后。。。");
		 $.ajax({
		        url: base+"bf/remarks/updateRemarksTH.nut",
		        data: { data: json },
		        type: "post",
		        success: function (text) {
		        	if(text.success){
...
function cancelRemark(){
	...
	$.ajax({
		type : "POST",
		url : base + "bf/remarks/cancelRemarks.nut",
		data : {"ids" : ids},
		dataType : 'json',
		success : function(data) {
			if (data.success) {
...
function agreeNCancel(){
	...
	$.ajax({
		type : "POST",
		url : base + "bf/remarks/agreeNCancel.nut",
		data : {"ids" : ids},
		dataType : 'json',
		success : function(data) {
			if (data.success) {
...
function agreeCancel(){
	...
	$.ajax({
		type : "POST",
		url : base + "bf/remarks/agreeCancel.nut",
		data : {"ids" : ids},
		dataType : 'json',
		success : function(data) {
			if (data.success) {
