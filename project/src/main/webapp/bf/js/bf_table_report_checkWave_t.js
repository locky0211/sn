...
function doEditData(obj) {
	...
	if (dType == 'LS') {
		...
		if (checkNum(obj)) {
			if (preStr == '%') {
				if (parseFloat(obj.val()).toFixed(4) != parseFloat(nobrText)
						.toFixed(4)) {// 修改过
					$("#isChanged").val("y");// 标识页面数据已修改
					obj.val(parseFloat(obj.val()).toFixed(4));
					nobr.html("<font color='red'>" + formatNumber(obj.val(), 4)
							+ preStr + "</font>");
				}
			} else {
				if (parseFloat(obj.val()).toFixed(2) != parseFloat(nobrText)
						.toFixed(2)) {// 修改过
					$("#isChanged").val("y");// 标识页面数据已修改
					obj.val(parseFloat(obj.val()).toFixed(2));
					nobr.html("<font color='red'>" + formatNumber(obj.val(), 2)
							+ "</font>");
				}
			}
...
function showStr(obj) {
	...
	if (preStr == '%') {
		if (parseFloat(cvalue).toFixed(4) != parseFloat(nobrText).toFixed(4)) {// 修改过
			...
			nobr.html("<font color='red'>" + formatNumber(cvalue, 4) + preStr
					+ "</font>");
		}
	} else {
		if (parseFloat(cvalue).toFixed(2) != parseFloat(nobrText).toFixed(2)) {// 修改过
			...
			nobr.html("<font color='red'>" + formatNumber(cvalue, 2)
					+ "</font>");
		}
	}
}
...
function doExportExcel() {
	var url = base + 'bf/table/report/temp/toExportTableReportExcel.nut?reportId='
			+ $('#reportId').val();
	mini.mask({
		el : document.body,
		cls : 'mini-mask-loading',
...
function doTableReportImport() {
	mini.open({
		url : base + 'bf/jsp/rd_table_report_import_t.jsp?reportId='
				+ $('#reportId').val(),
		title : '数据导入',
		width : 366,
		height : 198,
...
function changeValue() {
	...
	if (reg.test(tdtxt.replace(/,/g, ""))) {
		mini.open({
			url : base + 'bf/jsp/bf_table_report_changeValue_t.jsp?&value='
					+ tdtxt + '&inputId=' + inputId + '' + '&reportId='
					+ reportId + '&reportDate=' + reportDate,
			title : '数据修改',
			width : 300,
			height : 140,
			allowResize : false,
...
function show() {
	...
	if (reg.test(tdtxt)) {
		$.ajax({
			type : 'POST',
			url : base + 'bf/table/report/temp/showIndExplain.nut?value=' + tdtxt
					+ '&inputId=' + inputId,
			dataType : 'json',
			success : function(text) {
				if (text.data != null) {
...
function tips(id, str) {
	...
	t = getTop(document.getElementById(id))
