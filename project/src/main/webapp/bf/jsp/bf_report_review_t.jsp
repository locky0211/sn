...
<%HttpSession Session = request.getSession();
  ...
  if(list.contains("5")){
	...
	%>
<script type="text/javascript" src="../js/bf_report_review_t.js"></script>
...
<body>
				...
				<tr>
					<td style="width: 100%;"><label style="font-family: Verdana;"> 机构: </label> <input id="bmCode" showClose="true" oncloseclick="onCloseClick" name="brNo" style="width: 250px;"
						...
						textField="statusName" valueField="statusValue" data="[{statusValue:'0',statusName:'待审核'},{statusValue:'1',statusName:'审核未通过'},{statusValue:'2',statusName:'审核通过'}]" allowInput="false" value="${param.checkType }" />  
						  ...
						  <%if(list.contains("5")||list.contains("admin")){%>&nbsp;&nbsp;&nbsp;<a class="mini-button" iconCls="icon-txt" onclick="updateRemark()" style="color: green">提交备注</a>
					     <%};%>
						<%if(list.contains("6")||list.contains("admin")){%>
						...
						&nbsp;&nbsp;<a id='reviewButton' class="mini-button" iconCls="icon-txt" onclick="reviewSuccess()" style="color: green">审核通过</a>
						&nbsp;&nbsp;<a  class="mini-button" iconCls="icon-txt" onclick="deleteRemarks()" style="color: red">去除记录</a>
						<%};%>
						<%if(list.contains("5")||list.contains("admin")){%>
	...
	<div id="remarksResultGrid" allowAlternating="true" class="mini-datagrid" onrowdblclick="showCheckView" style="width: 100%; height: 100%;" url="${base}/bf/remarks/temp/getBfReportRemarksResultListArea.nut"
			...
			<div type="checkcolumn" width="20" align="center" headerAlign="center"></div>
				...
				<div field="remarks" headerAlign="center" renderer="onColor">备注信息<%if(list.contains("5")||list.contains("admin")) {%><input property="editor" class="mini-textarea" style="width:100%;"/><%};%></div>
				<div field="reportDate" headerAlign="center" >报表日期</div>
				<div field="reportReviewer" headerAlign="center" >审核员</div>
				<div field="reviewAdvice" headerAlign="center">审核意见<%if(list.contains("6")||list.contains("admin")){%><input property="editor" class="mini-textarea" style="width:100%;"/><%};%></div>
