...
function excute() {
	if (mini.get("brNo").getValue() == '' || mini.get("brNo").getValue() == null) {
		...
		$.ajax({
			type : 'POST',
			dataType : 'json',
			url : base + 'dck/base/startCheck.nut?brNo=' + mini.get("brNo").getValue() + '&reportDate=' + mini.get("reportDate").getText(),
			success : function(text) {
				if (text.success) {
...
function exportExcel() {
	...
	$.ajax({
		...
		data :"brnoName=" + brnoName+"&brno="+brno+"&reportDate="+reportDate+"&tableName=DCK.CHECK_BASE",
		dataType : 'json',
...
function searchCheckLog(){
	var brNo = mini.get("brNo").getValue();
	mini.open({
		url : base + 'dck/jsp/dckChecklog.jsp?brNo='+brNo+'&checkType='+1,
		title : "校验日志",
		width : 1000,
		height : 400,
		allowResize : true
...
function restartFormula() {
	...
	$.ajax({
		type : 'POST',
		dataType : 'json',
		url : base + 'dck/base/startSingleCheck.nut?brNo=' + mini.get("brNo").getValue() + '&reportDate=' + mini.get("reportDate").getText() + "&id=" +e.id + "&formulaId="+ e.formulaId,
		success : function(text) {
			if (text.success) {
				mini.alert(text.data, '提醒', function() {
					checkGrid.load({
