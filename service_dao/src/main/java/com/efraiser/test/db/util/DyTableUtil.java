...
public class DyTableUtil {
    ...
    public static boolean checkCellIsFormulaCellXlsx(XSSFCell cell) {
    ...
    public static String getLastReportDateWJ(String reportDate, String checkType) {
        int addMonth = 1;
        int addDay = 0;
        if ("D".equals(checkType)) {
            addDay += -1;
        } else if ("T".equals(checkType)) {
            addDay += -10;
        } else if ("M".equals(checkType)) {// 月
            addMonth += -1;
        } else if ("S".equals(checkType)) {// 季
            addMonth += -3;
        } else if ("SY".equals(checkType)) {// 半年
            addMonth += -6;
        ...
        if (addMonth != 1) {
            Date raw = DateUtil.parse(reportDate.substring(0, 6), "yyyyMM");
            raw = DateUtil.addMonth(raw, addMonth);
            System.out.println(DateUtil.format(raw, "yyyyMM"));
            return DateUtil.getLastMonthDay(DateUtil.format(raw, "yyyyMM") + "01");
        } else {
            Date raw = DateUtil.parse(reportDate, "yyyyMMdd");
            raw = DateUtil.addDay(raw, addDay);
            System.out.println(DateUtil.format(raw, "yyyyMMdd"));
            return DateUtil.format(raw, "yyyyMMdd");
        }
    }
    ...
    public static String getReportDateEndOfYearWJ(String reportDate) {
        Date raw = DateUtil.parse(reportDate, "yyyyMMdd");
        int addMonth = DateUtil.getMonth(raw) * -1;
        return DateUtil.getLastDay(DateUtil.format(DateUtil.addMonth(raw, addMonth), "yyyyMMdd"));
    }
    ...
    public static String getLastYearReportDateWJ(String reportDate) {
        Date raw = DateUtil.parse(reportDate, "yyyyMMdd");
        return DateUtil.format(DateUtil.addYear(raw, -1), "yyyyMMdd");
    }
    ...
    public String delParentNode(String brNos) {
        DyReportInfoServiceImpl rdReportInfoDao = (DyReportInfoServiceImpl) dyReportInfoService;
        String brNo = "";
    ...
    public static String getLastReportDateForWave(String reportDate,
                                                  String tabType) {
        int addMonth = 0;
