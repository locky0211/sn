package com.efraiser.test.db.service.dy.dytablecolumncontrast.impl;
...
@Service
public class DyTableColumnContrastServiceImpl extends BaseServiceImpl<DyTableColumnContrast> implements DyTableColumnContrastService {

    @Override
    public List<Integer> getColNames(String tableId, Integer part, List<Integer> valueCols) {
        Sql sql = Sqls.create("SELECT FILE_INDEX FROM DY.DY_TABLE_COLUMN_CONTRAST WHERE TABLE_ID=@tableId AND PART_NUM=@partNum AND COL_NUM IN ($colNums)");
    ...
    public Map<String, String> getColNumMap(String tableId) {
        Sql sql = Sqls.create("SELECT PART_NUM,COL_NUM,FILE_INDEX FROM DY.DY_TABLE_COLUMN_CONTRAST WHERE TABLE_ID=@tableId");
        sql.params().set("tableId", tableId);
    ...
    @Override
    public List<DyTableColumnContrast> getDyTableColumnContrastList(String tableId, int partNum) {
    ...
    @Override
    public List<DyTableColumnContrast> getAll() {
        String sqlStr = "SELECT * FROM DY.DY_TABLE_COLUMN_CONTRAST";
        List<DyTableColumnContrast> list = super.getListBySql(sqlStr, null, null);
        return list;
    }

    @Override
    public void delAll() {
        String sqlStr = "DELETE FROM DY.DY_TABLE_COLUMN_CONTRAST";
        Sql sql = Sqls.create(sqlStr);
        dao().execute(sql);
    }

    @Override
    public void add(DyTableColumnContrast item) {
        dao().insert(item);
    }
