package com.efraiser.test.db.model.dy;

import org.nutz.dao.entity.annotation.Column;
import org.nutz.dao.entity.annotation.Name;
import org.nutz.dao.entity.annotation.Table;

@Table("DY.DY_REPORT_DATA_SUMMARY")
public class DyReportDataSummary {
	@Name
	@Column("REPORT_ID")
	private String reportId;
	@Column("REPORT_ROW_NUM")
	private int reportRowNum;
	@Column("VALUE_A")
	private Double valueA;
	@Column("VALUE_B")
	private Double valueB;
	@Column("VALUE_C")
	private Double valueC;
	@Column("VALUE_D")
	private Double valueD;
	@Column("VALUE_E")
	private Double valueE;
	@Column("VALUE_F")
	private Double valueF;
	@Column("VALUE_G")
	private Double valueG;
	@Column("VALUE_H")
	private Double valueH;
	@Column("VALUE_I")
	private Double valueI;
	@Column("VALUE_J")
	private Double valueJ;
	@Column("VALUE_K")
	private Double valueK;
	@Column("VALUE_L")
	private Double valueL;
	@Column("VALUE_M")
	private Double valueM;
	@Column("VALUE_N")
	private Double valueN;
	@Column("VALUE_O")
	private Double valueO;
	@Column("VALUE_P")
	private Double valueP;
	@Column("VALUE_Q")
	private Double valueQ;
	@Column("VALUE_R")
	private Double valueR;
	@Column("VALUE_S")
	private Double valueS;
	@Column("VALUE_T")
	private Double valueT;
	@Column("VALUE_U")
	private Double valueU;
	@Column("VALUE_V")
	private Double valueV;
	@Column("VALUE_W")
	private Double valueW;
	@Column("VALUE_X")
	private Double valueX;
	@Column("VALUE_Y")
	private Double valueY;
	@Column("VALUE_Z")
	private Double valueZ;
	@Column("VALUE_AA")
	private Double valueAA;
	@Column("VALUE_AB")
	private Double valueAB;
	@Column("VALUE_AC")
	private Double valueAC;
	@Column("VALUE_AD")
	private Double valueAD;
	@Column("VALUE_AE")
	private Double valueAE;
	@Column("VALUE_AF")
	private Double valueAF;
	@Column("VALUE_AG")
	private Double valueAG;
	@Column("VALUE_AH")
	private Double valueAH;
	@Column("VALUE_AI")
	private Double valueAI;
	@Column("VALUE_AJ")
	private Double valueAJ;
	@Column("VALUE_AK")
	private Double valueAK;
	@Column("VALUE_AL")
	private Double valueAL;
	@Column("VALUE_AM")
	private Double valueAM;
	@Column("VALUE_AN")
	private Double valueAN;
	@Column("VALUE_AO")
	private Double valueAO;
	@Column("VALUE_AP")
	private Double valueAP;
	@Column("VALUE_AQ")
	private Double valueAQ;
	@Column("VALUE_AR")
	private Double valueAR;
	@Column("VALUE_AS")
	private Double valueAS;
	@Column("VALUE_AT")
	private Double valueAT;
	@Column("VALUE_AU")
	private Double valueAU;
	@Column("VALUE_AV")
	private Double valueAV;
	@Column("VALUE_AW")
	private Double valueAW;
	@Column("VALUE_AX")
	private Double valueAX;
	@Column("VALUE_AY")
	private Double valueAY;
	@Column("VALUE_AZ")
	private Double valueAZ;
	@Column("NAME_A")
	private String nameA;
	@Column("NAME_B")
	private String nameB;
	@Column("NAME_C")
	private String nameC;
	@Column("NAME_D")
	private String nameD;
	@Column("NAME_E")
	private String nameE;
	@Column("NAME_F")
	private String nameF;
	@Column("NAME_G")
	private String nameG;
	@Column("NAME_H")
	private String nameH;
	@Column("NAME_I")
	private String nameI;
	@Column("NAME_J")
	private String nameJ;
	@Column("NAME_K")
	private String nameK;
	@Column("NAME_L")
	private String nameL;
	@Column("NAME_M")
	private String nameM;
	@Column("NAME_N")
	private String nameN;
	@Column("NAME_O")
	private String nameO;
	@Column("NAME_P")
	private String nameP;
	@Column("NAME_Q")
	private String nameQ;
	@Column("NAME_R")
	private String nameR;
	@Column("NAME_S")
	private String nameS;
	@Column("NAME_T")
	private String nameT;
	@Column("NAME_U")
	private String nameU;
	@Column("NAME_V")
	private String nameV;
	@Column("NAME_W")
	private String nameW;
	@Column("NAME_X")
	private String nameX;
	@Column("NAME_Y")
	private String nameY;
	@Column("NAME_Z")
	private String nameZ;
	@Column("NAME_AA")
	private String nameAA;
	@Column("NAME_AB")
	private String nameAB;
	@Column("NAME_AC")
	private String nameAC;
	@Column("NAME_AD")
	private String nameAD;
	@Column("NAME_AE")
	private String nameAE;
	@Column("NAME_AF")
	private String nameAF;
	@Column("NAME_AG")
	private String nameAG;
	@Column("NAME_AH")
	private String nameAH;
	@Column("NAME_AI")
	private String nameAI;
	@Column("NAME_AJ")
	private String nameAJ;
	@Column("NAME_AK")
	private String nameAK;
	@Column("NAME_AL")
	private String nameAL;
	@Column("NAME_AM")
	private String nameAM;
	@Column("NAME_AN")
	private String nameAN;
	@Column("NAME_AO")
	private String nameAO;
	@Column("NAME_AP")
	private String nameAP;
	@Column("NAME_AQ")
	private String nameAQ;
	@Column("NAME_AR")
	private String nameAR;
	@Column("NAME_AS")
	private String nameAS;
	@Column("NAME_AT")
	private String nameAT;
	@Column("NAME_AU")
	private String nameAU;
	@Column("NAME_AV")
	private String nameAV;
	@Column("NAME_AW")
	private String nameAW;
	@Column("NAME_AX")
	private String nameAX;
	@Column("NAME_AY")
	private String nameAY;
	@Column("NAME_AZ")
	private String nameAZ;

	public String getReportId() {
		return reportId;
	}

	public void setReportId(String reportId) {
		this.reportId = reportId;
	}

	public int getReportRowNum() {
		return reportRowNum;
	}

	public void setReportRowNum(int reportRowNum) {
		this.reportRowNum = reportRowNum;
	}

	public Double getValueA() {
		return valueA;
	}

	public void setValueA(Double valueA) {
		this.valueA = valueA;
	}

	public Double getValueB() {
		return valueB;
	}

	public void setValueB(Double valueB) {
		this.valueB = valueB;
	}

	public Double getValueC() {
		return valueC;
	}

	public void setValueC(Double valueC) {
		this.valueC = valueC;
	}

	public Double getValueD() {
		return valueD;
	}

	public void setValueD(Double valueD) {
		this.valueD = valueD;
	}

	public Double getValueE() {
		return valueE;
	}

	public void setValueE(Double valueE) {
		this.valueE = valueE;
	}

	public Double getValueF() {
		return valueF;
	}

	public void setValueF(Double valueF) {
		this.valueF = valueF;
	}

	public Double getValueG() {
		return valueG;
	}

	public void setValueG(Double valueG) {
		this.valueG = valueG;
	}

	public Double getValueH() {
		return valueH;
	}

	public void setValueH(Double valueH) {
		this.valueH = valueH;
	}

	public Double getValueI() {
		return valueI;
	}

	public void setValueI(Double valueI) {
		this.valueI = valueI;
	}

	public Double getValueJ() {
		return valueJ;
	}

	public void setValueJ(Double valueJ) {
		this.valueJ = valueJ;
	}

	public Double getValueK() {
		return valueK;
	}

	public void setValueK(Double valueK) {
		this.valueK = valueK;
	}

	public Double getValueL() {
		return valueL;
	}

	public void setValueL(Double valueL) {
		this.valueL = valueL;
	}

	public Double getValueM() {
		return valueM;
	}

	public void setValueM(Double valueM) {
		this.valueM = valueM;
	}

	public Double getValueN() {
		return valueN;
	}

	public void setValueN(Double valueN) {
		this.valueN = valueN;
	}

	public Double getValueO() {
		return valueO;
	}

	public void setValueO(Double valueO) {
		this.valueO = valueO;
	}

	public Double getValueP() {
		return valueP;
	}

	public void setValueP(Double valueP) {
		this.valueP = valueP;
	}

	public Double getValueQ() {
		return valueQ;
	}

	public void setValueQ(Double valueQ) {
		this.valueQ = valueQ;
	}

	public Double getValueR() {
		return valueR;
	}

	public void setValueR(Double valueR) {
		this.valueR = valueR;
	}

	public Double getValueS() {
		return valueS;
	}

	public void setValueS(Double valueS) {
		this.valueS = valueS;
	}

	public Double getValueT() {
		return valueT;
	}

	public void setValueT(Double valueT) {
		this.valueT = valueT;
	}

	public Double getValueU() {
		return valueU;
	}

	public void setValueU(Double valueU) {
		this.valueU = valueU;
	}

	public Double getValueV() {
		return valueV;
	}

	public void setValueV(Double valueV) {
		this.valueV = valueV;
	}

	public Double getValueW() {
		return valueW;
	}

	public void setValueW(Double valueW) {
		this.valueW = valueW;
	}

	public Double getValueX() {
		return valueX;
	}

	public void setValueX(Double valueX) {
		this.valueX = valueX;
	}

	public Double getValueY() {
		return valueY;
	}

	public void setValueY(Double valueY) {
		this.valueY = valueY;
	}

	public Double getValueZ() {
		return valueZ;
	}

	public void setValueZ(Double valueZ) {
		this.valueZ = valueZ;
	}

	public Double getValueAA() {
		return valueAA;
	}

	public void setValueAA(Double valueAA) {
		this.valueAA = valueAA;
	}

	public Double getValueAB() {
		return valueAB;
	}

	public void setValueAB(Double valueAB) {
		this.valueAB = valueAB;
	}

	public Double getValueAC() {
		return valueAC;
	}

	public void setValueAC(Double valueAC) {
		this.valueAC = valueAC;
	}

	public Double getValueAD() {
		return valueAD;
	}

	public void setValueAD(Double valueAD) {
		this.valueAD = valueAD;
	}

	public Double getValueAE() {
		return valueAE;
	}

	public void setValueAE(Double valueAE) {
		this.valueAE = valueAE;
	}

	public Double getValueAF() {
		return valueAF;
	}

	public void setValueAF(Double valueAF) {
		this.valueAF = valueAF;
	}

	public Double getValueAG() {
		return valueAG;
	}

	public void setValueAG(Double valueAG) {
		this.valueAG = valueAG;
	}

	public Double getValueAH() {
		return valueAH;
	}

	public void setValueAH(Double valueAH) {
		this.valueAH = valueAH;
	}

	public Double getValueAI() {
		return valueAI;
	}

	public void setValueAI(Double valueAI) {
		this.valueAI = valueAI;
	}

	public Double getValueAJ() {
		return valueAJ;
	}

	public void setValueAJ(Double valueAJ) {
		this.valueAJ = valueAJ;
	}

	public Double getValueAK() {
		return valueAK;
	}

	public void setValueAK(Double valueAK) {
		this.valueAK = valueAK;
	}

	public Double getValueAL() {
		return valueAL;
	}

	public void setValueAL(Double valueAL) {
		this.valueAL = valueAL;
	}

	public Double getValueAM() {
		return valueAM;
	}

	public void setValueAM(Double valueAM) {
		this.valueAM = valueAM;
	}

	public Double getValueAN() {
		return valueAN;
	}

	public void setValueAN(Double valueAN) {
		this.valueAN = valueAN;
	}

	public String getNameA() {
		return nameA;
	}

	public void setNameA(String nameA) {
		this.nameA = nameA;
	}

	public String getNameB() {
		return nameB;
	}

	public void setNameB(String nameB) {
		this.nameB = nameB;
	}

	public String getNameC() {
		return nameC;
	}

	public void setNameC(String nameC) {
		this.nameC = nameC;
	}

	public String getNameD() {
		return nameD;
	}

	public void setNameD(String nameD) {
		this.nameD = nameD;
	}

	public String getNameE() {
		return nameE;
	}

	public void setNameE(String nameE) {
		this.nameE = nameE;
	}

	public String getNameF() {
		return nameF;
	}

	public void setNameF(String nameF) {
		this.nameF = nameF;
	}

	public String getNameG() {
		return nameG;
	}

	public void setNameG(String nameG) {
		this.nameG = nameG;
	}

	public String getNameH() {
		return nameH;
	}

	public void setNameH(String nameH) {
		this.nameH = nameH;
	}

	public String getNameI() {
		return nameI;
	}

	public void setNameI(String nameI) {
		this.nameI = nameI;
	}

	public String getNameJ() {
		return nameJ;
	}

	public void setNameJ(String nameJ) {
		this.nameJ = nameJ;
	}

	public String getNameK() {
		return nameK;
	}

	public void setNameK(String nameK) {
		this.nameK = nameK;
	}

	public String getNameL() {
		return nameL;
	}

	public void setNameL(String nameL) {
		this.nameL = nameL;
	}

	public String getNameM() {
		return nameM;
	}

	public void setNameM(String nameM) {
		this.nameM = nameM;
	}

	public String getNameN() {
		return nameN;
	}

	public void setNameN(String nameN) {
		this.nameN = nameN;
	}

	public String getNameO() {
		return nameO;
	}

	public void setNameO(String nameO) {
		this.nameO = nameO;
	}

	public String getNameP() {
		return nameP;
	}

	public void setNameP(String nameP) {
		this.nameP = nameP;
	}

	public String getNameQ() {
		return nameQ;
	}

	public void setNameQ(String nameQ) {
		this.nameQ = nameQ;
	}

	public String getNameR() {
		return nameR;
	}

	public void setNameR(String nameR) {
		this.nameR = nameR;
	}

	public String getNameS() {
		return nameS;
	}

	public void setNameS(String nameS) {
		this.nameS = nameS;
	}

	public String getNameT() {
		return nameT;
	}

	public void setNameT(String nameT) {
		this.nameT = nameT;
	}

	public String getNameU() {
		return nameU;
	}

	public void setNameU(String nameU) {
		this.nameU = nameU;
	}

	public String getNameV() {
		return nameV;
	}

	public void setNameV(String nameV) {
		this.nameV = nameV;
	}

	public String getNameW() {
		return nameW;
	}

	public void setNameW(String nameW) {
		this.nameW = nameW;
	}

	public String getNameX() {
		return nameX;
	}

	public void setNameX(String nameX) {
		this.nameX = nameX;
	}

	public String getNameY() {
		return nameY;
	}

	public void setNameY(String nameY) {
		this.nameY = nameY;
	}

	public String getNameZ() {
		return nameZ;
	}

	public void setNameZ(String nameZ) {
		this.nameZ = nameZ;
	}

	public String getNameAA() {
		return nameAA;
	}

	public void setNameAA(String nameAA) {
		this.nameAA = nameAA;
	}

	public String getNameAB() {
		return nameAB;
	}

	public void setNameAB(String nameAB) {
		this.nameAB = nameAB;
	}

	public String getNameAC() {
		return nameAC;
	}

	public void setNameAC(String nameAC) {
		this.nameAC = nameAC;
	}

	public String getNameAD() {
		return nameAD;
	}

	public void setNameAD(String nameAD) {
		this.nameAD = nameAD;
	}

	public String getNameAE() {
		return nameAE;
	}

	public void setNameAE(String nameAE) {
		this.nameAE = nameAE;
	}

	public String getNameAF() {
		return nameAF;
	}

	public void setNameAF(String nameAF) {
		this.nameAF = nameAF;
	}

	public String getNameAG() {
		return nameAG;
	}

	public void setNameAG(String nameAG) {
		this.nameAG = nameAG;
	}

	public String getNameAH() {
		return nameAH;
	}

	public void setNameAH(String nameAH) {
		this.nameAH = nameAH;
	}

	public String getNameAI() {
		return nameAI;
	}

	public void setNameAI(String nameAI) {
		this.nameAI = nameAI;
	}

	public String getNameAJ() {
		return nameAJ;
	}

	public void setNameAJ(String nameAJ) {
		this.nameAJ = nameAJ;
	}

	public String getNameAK() {
		return nameAK;
	}

	public void setNameAK(String nameAK) {
		this.nameAK = nameAK;
	}

	public String getNameAL() {
		return nameAL;
	}

	public void setNameAL(String nameAL) {
		this.nameAL = nameAL;
	}

	public String getNameAM() {
		return nameAM;
	}

	public void setNameAM(String nameAM) {
		this.nameAM = nameAM;
	}

	public String getNameAN() {
		return nameAN;
	}

	public void setNameAN(String nameAN) {
		this.nameAN = nameAN;
	}
	
	public Double getValueAO() {
		return valueAO;
	}

	public void setValueAO(Double valueAO) {
		this.valueAO = valueAO;
	}

	public Double getValueAP() {
		return valueAP;
	}

	public void setValueAP(Double valueAP) {
		this.valueAP = valueAP;
	}

	public Double getValueAQ() {
		return valueAQ;
	}

	public void setValueAQ(Double valueAQ) {
		this.valueAQ = valueAQ;
	}

	public Double getValueAR() {
		return valueAR;
	}

	public void setValueAR(Double valueAR) {
		this.valueAR = valueAR;
	}

	public Double getValueAS() {
		return valueAS;
	}

	public void setValueAS(Double valueAS) {
		this.valueAS = valueAS;
	}

	public Double getValueAT() {
		return valueAT;
	}

	public void setValueAT(Double valueAT) {
		this.valueAT = valueAT;
	}

	public Double getValueAU() {
		return valueAU;
	}

	public void setValueAU(Double valueAU) {
		this.valueAU = valueAU;
	}

	public Double getValueAV() {
		return valueAV;
	}

	public void setValueAV(Double valueAV) {
		this.valueAV = valueAV;
	}

	public Double getValueAW() {
		return valueAW;
	}

	public void setValueAW(Double valueAW) {
		this.valueAW = valueAW;
	}

	public Double getValueAX() {
		return valueAX;
	}

	public void setValueAX(Double valueAX) {
		this.valueAX = valueAX;
	}

	public Double getValueAY() {
		return valueAY;
	}

	public void setValueAY(Double valueAY) {
		this.valueAY = valueAY;
	}

	public Double getValueAZ() {
		return valueAZ;
	}

	public void setValueAZ(Double valueAZ) {
		this.valueAZ = valueAZ;
	}

	public String getNameAO() {
		return nameAO;
	}

	public void setNameAO(String nameAO) {
		this.nameAO = nameAO;
	}

	public String getNameAP() {
		return nameAP;
	}

	public void setNameAP(String nameAP) {
		this.nameAP = nameAP;
	}

	public String getNameAQ() {
		return nameAQ;
	}

	public void setNameAQ(String nameAQ) {
		this.nameAQ = nameAQ;
	}

	public String getNameAR() {
		return nameAR;
	}

	public void setNameAR(String nameAR) {
		this.nameAR = nameAR;
	}

	public String getNameAS() {
		return nameAS;
	}

	public void setNameAS(String nameAS) {
		this.nameAS = nameAS;
	}

	public String getNameAT() {
		return nameAT;
	}

	public void setNameAT(String nameAT) {
		this.nameAT = nameAT;
	}

	public String getNameAU() {
		return nameAU;
	}

	public void setNameAU(String nameAU) {
		this.nameAU = nameAU;
	}

	public String getNameAV() {
		return nameAV;
	}

	public void setNameAV(String nameAV) {
		this.nameAV = nameAV;
	}

	public String getNameAW() {
		return nameAW;
	}

	public void setNameAW(String nameAW) {
		this.nameAW = nameAW;
	}

	public String getNameAX() {
		return nameAX;
	}

	public void setNameAX(String nameAX) {
		this.nameAX = nameAX;
	}

	public String getNameAY() {
		return nameAY;
	}

	public void setNameAY(String nameAY) {
		this.nameAY = nameAY;
	}

	public String getNameAZ() {
		return nameAZ;
	}

	public void setNameAZ(String nameAZ) {
		this.nameAZ = nameAZ;
	}

}
